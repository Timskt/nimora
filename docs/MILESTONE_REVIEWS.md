...
- 原生闭环：恢复交互成功后复用既有多屏可见位置规划，将离屏窗口夹回最大重叠显示器或主显示器安全边界；移动事件把最终坐标同步回 Runtime。
- 失联闭环：若 Pet WebView 已不存在，托盘操作不会停在 `WindowUnavailable`；它先提交可见且可交互的权威恢复意图，再启动受三次/分钟预算和关停门禁保护的唯一 Recovery Worker。重建窗口会在启用鼠标交互前复验旧坐标，失败销毁半成品并进入下一次有界重试。
- 原子创建：原生 `build()` 之后的坐标、Work Area 与点击穿透配置共享成功/清理边界；只有全部成功才记录新心跳。故障注入测试证明失败必清理、成功不误销毁，避免现存窗口标签让 Worker 错误短路。
- 架构边界：召回只由 Desktop Coordinator 执行，事件只记录 `positionRecovered` 布尔事实，不把显示器身份、坐标或桌面内容升级为活动遥测。
- 验证边界：纯几何测试证明负坐标、多显示器、主屏回退和分辨率缩小；真实 Dock/任务栏动态 Work Area、混合 DPI 与热插拔视觉必须继续使用签名双平台包验证。

## 2026-07-19 — 活动健康刷新闭环

- 缺口修正：活动页此前承诺“自动更新”，实际只在应用启动读取一次 Outbox，长时间驻留会展示陈旧健康状态。
- 生命周期：仅活动页且文档可见时刷新，进入与恢复可见立即读取，前台每 10 秒读取；离开页面完整清理，慢请求不会并发堆积。
- 稳定性：失败保留上次快照并提示降级状态，不把暂时读取失败误表示为零，不增加网络依赖或第二套活动存储。
- 证据：纯函数覆盖活动/可见性门控，Frontend 106 项测试和 TypeScript 通过；Chrome 810px 截图、无横向溢出、可访问树与零 Console Error 通过。
- 边界：浏览器证据只验证控制中心 UI；原生透明合成、置顶、Work Area、DPI 与桌宠窗口行为仍必须由 Tauri 真机门禁证明。

## 2026-07-19 — 控制中心可见入口完整性复盘

- 缺口修正：头像按钮和概览“查看全部”此前只有视觉 affordance 没有行为；更严重的是一级“活动”会落入默认分支再次显示概览，造成导航承诺与内容不一致。
- 功能闭环：新增真正的活动工作区，三个入口分别到达活动或设置，当前一级项通过 `aria-current` 暴露给辅助技术；不以移除按钮掩盖功能缺失。
- 数据边界：活动页只投影已有 Outbox 有界计数与固定健康描述，不新增网络遥测、正文日志或并行事件模型；未来扩展必须继续通过脱敏 Activity Projection，而非直接暴露事件载荷。
- 视觉验证：810px 浏览器截图证明摘要层级、健康列表和本地标记清晰，页面无横向溢出或 Console Error；窄宽度与签名原生窗口继续复用对应发布门禁。

## 2026-07-19 — 控制中心窄宽度重排复盘

- 缺口修正：全局 `body min-width:720px` 与“720px 仍保留固定侧栏”的旧规范会在 200% 缩放或较小桌面窗口制造页面级横向滚动，违背既有 UI-003。
- 设计修正：宽屏布局完全保留；720px 以下把信息架构从左右栏重排为顶部品牌/状态、横向主导航和单列工作区，480px 以下再分行顶栏与 Hero 操作，不删除任何一级能力。
- 鲁棒性：导航自身承担有界横向滚动与滚动链隔离，工作区恢复自然溢出，避免用 `overflow:hidden` 把尚未适配的子页面悄悄裁掉；桌宠独立窗口样式保持隔离。
- 证据边界：810px 浏览器截图与 Console 证明宽屏无回归，但当前插件不能改变视口或缩放，不能宣称 720/480px 视觉已通过；真实窄宽度、系统文字缩放和平台字体继续列为门禁。

## 2026-07-19 — 桌宠完整菜单键盘可达性复盘

- 缺口修正：此前完整径向菜单只有鼠标右键和触摸/指针长按入口；隐藏快捷栏虽可聚焦基础操作，但背包、回家、改名和边缘动作对纯键盘用户不可发现。
- 输入一致性：新增平台通用菜单键与 `Shift+F10`，复用同一菜单状态机，不复制动作或绕过 Capability；Enter/Space 仍是陪伴互动，避免破坏既有肌肉记忆。
- 焦点闭环：打开后首项确定性聚焦，方向键继续环形移动，Esc 关闭并把焦点归还桌宠；浏览器截图与可访问树已验证 260×300 验收框内无裁切。
- 证据边界：Browser Preview 只证明 DOM、焦点和视觉几何，不冒充 Windows 菜单键分派、macOS 键盘映射或透明置顶窗口证据；这些继续进入签名原生包门禁。

## 2026-07-19 — Reduced Motion 跨层一致性复盘

- 不足修正：此前 CSS、Sprite 与 glTF/VRM 会减少动画，但 Desktop Host 的 12 帧窗口漫游不读取系统偏好，用户仍会看到整只桌宠在屏幕上移动。
- 架构修正：新增只含布尔值的 Preference Intent；WebView 负责感知，Desktop Coordinator 负责原生执行，避免破坏唯一窗口执行者原则。
- 产品边界：减少非必要运动而非禁用宠物；生命状态、照料、键盘操作、主动拖拽与回家保持可用，也不把设备偏好持久化进 Profile。
- 证据边界：订阅、IPC 与双平台编译可自动验证；真实系统设置、GPU Renderer 和窗口轨迹仍保留签名真机门禁。

## 2026-07-19 — 系统唤醒恢复协议复盘

- 不足修正：此前一秒可见性守卫只能处理仍存在且策略可见的窗口，不能证明休眠期间窗口丢失、原生置顶/穿透状态失效或退出竞态下的行为。
- 架构修正：Resume 与 Shutdown 复用同一激活 gate，恢复只重放权威 Window Policy，不从 WebView 私有状态猜测；缺失窗口进入既有 Recovery Host。
- 用户意图：隐藏策略优先于“常驻”目标，唤醒不得擅自显示、抢焦点或打开控制中心；可见位置修复不依赖 AI、Provider 或网络。
- 证据边界：规划单测和双平台编译只能证明分流与可移植性，真实休眠、DPI、显示器热插拔和 WebView 回收继续保留签名真机门禁。
...
- 协调边界：托盘在退出调用前发布不可逆 shutdown intent；全局退出事件幂等复用，并负责耗时能力面的有界静默，避免把退出关键路径散落到 Renderer 或扩展。
- 数据边界：最终桌宠位置先尽力同步保存，失败可审计但不绑架退出；不会为了可靠性把 AI、网络或人工确认加入关停前提。
- 证据边界：恢复宿主自动化覆盖“shutdown 后拒绝新 worker”，双平台签名包仍需验证真实销毁顺序、孤儿进程与重启落点。
- 数据修正：旧同步刷新复用了“拖拽中跳过”的防抖规则，会把跳过伪装成保存成功；现以显式持久化意图区分后台移动与关停刷新，并在退出中原子结束持久 `Dragged` 状态。
- 入口修正：仅托盘执行关停刷新仍会让 `⌘Q`、会话结束等系统入口丢失最后 200ms 落点；`ExitRequested` 现幂等复用 `ShutdownFlush`，失败可诊断但绝不阻断操作系统退出。

## 2026-07-19 — 常驻应用重开可达性复盘

- 缺口修正：控制中心关闭语义是隐藏而非退出，但此前 macOS Dock 重开事件未处理，会造成“桌宠仍在、管理入口打不开”的可达性故障。
- 架构复用：`Reopen` 继续走 Desktop Host 唯一窗口恢复原语和同一审计入口；Renderer、Sensor、AI 与扩展不获得原生窗口执行权。
- 稳定边界：其他可见窗口不阻止恢复，重复激活不创建资源；恢复或审计失败不结束 Runtime，也不把非关键诊断写入升级为桌宠退出条件。
- 平台边界：macOS 使用系统重开事件，Windows 保持单实例激活；签名真机焦点与 Dock/任务栏行为仍需发布门禁，浏览器预览不能替代。

## 2026-07-19 — 单实例常驻一致性复盘

- 风险修正：登录启动上线后，用户重复点击会把多实例从边缘问题升级为数据一致性和体验问题，因此单实例是常驻能力的必要配套，而非后续优化。
- 复用边界：第二实例复用现有控制中心恢复事务，继续统一显示、取消最小化、聚焦与审计，不新增旁路窗口协调器。
- 输入安全：启动参数与工作目录不具备隐式深链权限，当前全部丢弃；未来协议扩展必须经过版本化解析、来源约束和 Capability Admission。
- 证据边界：交叉编译只能证明平台 API 可构建，进程互斥、用户会话与窗口聚焦必须在签名安装包上验证。

## 2026-07-19 — 原生常驻体验复盘

- 产品修正：QQ 宠物式陪伴不能依赖用户每天手动打开，新增显式选择的登录后启动，同时坚持默认关闭与可撤销。
- 架构边界：采用官方系统集成而非自造 LaunchAgent/注册表；控制中心独占 capability，桌宠渲染与扩展生态不能触碰登录项。
- 稳定性：系统状态是唯一权威，写后复查并处理外部修改；失败不改变已确认 UI 状态，不让重复偏好数据库与操作系统漂移。
- 离线与安全：登录启动只启动基础宿主，不顺带唤醒任何 AI、网络或用户代码执行面，避免“常驻”演变为隐式授权。
- 证据边界：编译、权限清单与自动化测试不能替代重登和卸载真机验证；macOS/Windows 发布前必须完成对应门禁。
- 体验修正：首次实现若沿用 Tauri 默认可见窗口，会在每次登录弹出 1180×780 控制中心；现从配置层默认隐藏，并由宿主依据严格启动意图决定显示，消除启动闪烁和抢焦点。
...
- 领域：`resumeAction` 是自主事务的受限恢复点，不是第二份 Surface 状态；只允许 Perch/Climb/Peek，拖拽抢占丢弃，Quiet/Focus 抑制恢复。
- 原生边界：路径只由 Desktop Coordinator 基于当前 Work Area 规划，Corner 归属唯一，端点反向，负坐标与生命体安全边距保持正确。
- 生态：Schema 向后兼容缺失字段，离线自主循环不依赖 AI；Renderer、Program、Skill、Agent 和 Sensor 均未获得移窗能力。
- 验证：领域恢复/恶意快照、四边路径、Corner、端点与负坐标均有定向测试；签名真机的动画连续性、动态 Dock/任务栏与混合 DPI 仍需后续证据。
- Actions：继续使用本地全量门禁后集中推送，避免无价值触发 GitHub Actions 月度时长。
...
- 架构：平台几何只由 Tauri Host 获取并转换为封闭的物理矩形，所有位置算法复用同一安全边界；Renderer、AI、Skill、Program 与 Sensor 不获得伪造系统栏尺寸或直接移动窗口的能力。
- 鲁棒性：负坐标副屏、不同工作区原点、最大交集选择和无显示器降级保持确定性；现有位置防抖、窗口事务与 Pet Core 持久权威不变。
- 验证：纯 Rust 测试覆盖左/上/下系统保留区及吸附、漫游、恢复；自动隐藏栏、混合 DPI、动态 Dock 和 Wayland 差异保留为各平台签名真机门禁。
...
## 2026-07-19 — Bounded Pet Action Menu

- 复盘发现“更多”页完整能力数已超过 260×300 内容区的静态高度；全新项目不以删功能规避，而是把所有非径向子页升级为窗口内有界滚动容器。
- 键盘焦点移动同步执行 nearest scroll，Home/End 与方向键可到达首尾动作；细滚动条、滚动链隔离和居中入场兼顾桌宠美感与可用性，根径向菜单保持原视觉模型。
- 前端 86 项测试、生产构建与 Bundle Budget 已通过；浏览器扩展本地标签接管超时被如实记录，不能替代后续 260×300 截图和签名原生窗口门禁。
...
# 2026-07-19 Profile 级鼠标关注控制回顾

- 偏差修正：若只能通过关闭全部自主互动来禁用靠近鼠标，会让易受动态干扰、演示和精确指针操作用户失去其它桌宠生命行为。
- 契约：新增可继承布尔偏好，旧数据默认开启；Profile Merge、Schema 解析、宿主决策、Preview 与 Creator 产物保持同一语义。
- 执行：宿主在光标 API 前检查活动 Profile；关闭时根本不采样，而不是采样后丢弃，并回退确定性 Free Wander。
- 体验：创建与编辑 Profile 均可独立控制，摘要明确当前状态，文案说明一次采样与不记录轨迹，不以模糊“隐私模式”误导用户。
- 扩展：未来其它自主子行为应沿用显式 Profile Policy + 宿主门禁模式，不向 Renderer 或用户代码下放系统输入与窗口能力。
- 视觉证据：Chrome 扩展可列出本地 Nimora 标签，但 `claimTab` 再次超时并重置，未取得可信 DOM 或截图；窄宽度、200% 缩放和原生 Profile 切换仍保留人工发布门禁。
## 2026-07-19 — QQ 宠物式主动表达复盘

- 偏差修正：原 Overlay 只有悬浮/聚焦状态标签，不足以形成 QQ 宠物式主动陪伴；现改为短时主动气泡，同时保留按需重看。
- 架构边界：没有为一句话扩大领域模型、数据库或 AI 权限；调度留在宿主 UI，Renderer、Skill 和用户程序不能绕过模板与能力网关。
- 打扰控制：菜单、拖拽和抚摸优先于表达，气泡自动消退且不发系统通知；后续长文本必须进入独立非模态面板。
- 鲁棒性修正：从字符串状态升级为带修订号的调度器，重复互动反馈可可靠重播；Status 有 8 秒冷却且不能覆盖 Feedback/Error。
- 美学修正：原胶囊状态标签缺少对白指向和长文案边界，现使用受控 Token、14px 气泡、尾巴、42 字缩略、双侧安全区及 Forced Colors 降级。
- 证据边界：自动化覆盖策略与前端构建；透明合成、裁切、跨平台字体和缩放仍以 Tauri 真机截图为发布门禁。

## 2026-07-19 — Profile 自主气泡控制复盘

- 需求修正：用户需要安静时不应被迫关闭全部自主行为；新增独立表现偏好，只静音自主状态表达。
- 架构结论：Profile Repository 仍由宿主持有，Pet Overlay 只消费 Desktop Snapshot；变更事件仅作刷新提示，不携带可伪造策略。
- 反馈安全：关闭策略立即撤下 Status，但 Feedback/Error 永不随之静音，避免安静设置掩盖操作结果与故障。
- 兼容与扩展：旧 Profile 默认开启，Creator、Preview 与原生宿主共享 Schema；未来新增表达通道必须先明确优先级和是否可静音。
- 证据边界：自动化覆盖迁移、合并、调度和类型契约；透明原生窗口中的切换观感仍需 macOS/Windows 真机截图验收。

## 2026-07-19 — 控制中心中等视口视觉复盘

- 发现：真实 810×607 浏览器审查暴露 Hero 标题“里。”孤立成行，源码与自动化测试无法替代该视觉证据。
- 修正：新增 860px 中间断点，适度收窄并缩放角色列、扩大文本列，同时保持侧栏、角色和操作层级不变。
- 取舍：不硬编码中文换行、不重做成熟视觉体系，也不为窄视口隐藏桌宠；规则对未来本地化文案和新角色资产继续适用。
- 结果：复核截图中标题、正文、五个操作和 Aster 均无拥挤或遮挡；原生窗口特性仍由独立 Tauri 门禁负责。

## 2026-07-19 — Agent 任务协作成长复盘

- 对齐：补齐能力矩阵明确缺失的“任务协作成长”，让桌宠参与真实工作成果，而不是只播放不持久的 Agent 状态动画。
- 信任：成长资格由宿主 Agent Task 元数据判定；Renderer 事件、模块代理和第三方扩展不能伪造成功领取关系点。
- 一致性：Task UUID 收据直接进入 Pet 快照，避免 Agent History 与 Pet Repository 双写；重复完成、保存失败和纪念跨阈值均保持确定性。
- 低压力：每个真实任务固定小额成长，无签到、连续任务加成、失败惩罚或 Token/费用挂钩，离线内置 Provider 同样有效。
- 扩展：未来 Goal、Auto Mode 与多人协作奖励必须定义独立可信完成凭证和防刷策略，不能复用前端状态信号或绕过 Pet 领域事务。
## 2026-07-19 — 自主行为持久频率预算复盘

- 问题：仅有 Idle Delay 和 Cooldown 会让高频 Profile 在长期运行中持续打扰，也会让重启成为隐式清空频率限制的途径。
- 方案：在 Pet 领域快照内加入平滑补充令牌桶，Profile 只选择产品频率，Desktop Host 映射容量和速率，Renderer、AI 与扩展不能直接写预算。
- 鲁棒性：旧快照安全初始化；时钟回拨不补充；异常容量被 Core 钳制；伪造 Start 无法透支；预算耗尽不影响用户主动操作。
- 取舍：不使用整点清零或无限历史时间戳，避免时间边界突发和快照无界增长；真实 24 小时节奏继续由签名桌面长稳门禁校准。

## 2026-07-19 — QQ 宠物式全局注意力预算复盘

- 偏差修正：自主气泡、Agent 陪伴与声音原先各自限流，多个模块可能同时打扰，且扩展生态缺少统一申请边界。
- 架构：Tauri Host 成为唯一 Attention Authority；来源、通道和优先级均为版本化封闭契约，Renderer、AI、Skill、Automation 与用户程序不能直接消费预算或操纵窗口。
- 体验：Ambient 共享一分钟 12 令牌并接受全屏、游戏与勿扰抑制；用户主动反馈和 Safety 始终优先，保持 QQ 宠物式“常驻但不纠缠”。
- 完整纵切：自主 Motion 在领域 Start 前裁决，自主/Agent 气泡和控制中心声音接入同一网关；拒绝不会先播放、先移动或写入用户内容。
- 验证：Rust 覆盖优先级、环境抑制、跨通道耗尽、重试与补充，Frontend 覆盖精确 IPC；真实多屏、会议和连续八小时打扰密度仍保留签名原生包门禁。

## 2026-07-19 — QQ 宠物式 WebView 自愈复盘

- 偏差修正：已有 Destroyed 恢复只能处理窗口消失，WebKit/WebView2 卡死但透明窗口壳仍存在时，桌宠会永久失去互动且宿主误认为健康。
- 架构：Overlay 只提交 15 秒无内容心跳；Tauri Host 以 90 秒阈值、可见性重新武装和既有单 Worker 恢复预算决定销毁与重建，Renderer 与前端没有恢复控制权。
- 鲁棒性：隐藏窗口不判死，重新显示获得完整宽限；迟到或回拨心跳不能倒退时间；Destroyed 与看门狗并发只允许一个恢复 Worker；shutdown intent 永久禁止复活。
- 隐私与离线：心跳不持久化、不上传、不含 Pet 状态、页面状态或用户内容，不依赖 Provider、Agent、控制中心或网络。
- 验证：策略测试覆盖阈值、回拨、隐藏/显示、退避与退出竞态；真实 WebKit/WebView2 进程终止、睡眠唤醒和八小时长稳仍必须由签名原生包证明，Browser Preview 不作为证据。

## 2026-07-19 — QQ 宠物式 Overlay 视觉复盘

- 结论：当前 Overlay 已形成“角色优先、控件按需出现”的桌宠层级，而不是把控制中心缩小后悬浮；默认画面只保留角色与瞬时对白。
- 交互：悬停工具条承载高频照料，右键/长按环形菜单承载分层能力，键盘以主角色按钮和 `Shift+F10` 获得等价路径。
