...
| 流程发现与能力挖掘 | 在用户明确选择的事件、操作记录和失败轨迹中发现重复流程、等待点和自动化机会 | Opportunity Report / Recipe Draft；本地分析优先、数据窗口可见、先建议后生成且禁止暗中监控 |
| 跨应用语义编排 | 把“联系人、项目、任务、文件、设备”等不同系统对象映射为稳定语义实体并生成跨应用流程 | Semantic Bridge / Workflow Graph；实体消歧、来源引用、写入目标预览、冲突合并与可撤销补偿 |
| 可验证生成与形式约束 | 从安全不变量、状态机和业务规则生成属性测试、模型检查输入、约束求解器规则与证明报告 | Verification Pack；证明范围显式、反例可回放、不得把模型自评当作证明 |
| 数据库与个人信息空间 | 为用户创建本地优先的数据表、关系、表单、视图、导入器和生命周期规则 | Personal Data App；Schema Diff、字段分级、事务迁移、备份恢复和逐项删除传播 |
| 团队协作与能力交接 | 把个人能力参数化为团队模板，生成角色分工、审批、共享配置和可移交维护包 | Collaboration Pack；个人数据剥离、组织策略上限、责任人、审计与离职撤权 |
| 能力组合优化 | 分析多个已安装能力的重复调用、冲突触发、资源竞争和可合并路径，并提出组合或拆分方案 | Composition Patch；调用图、等价性回放、性能预算、行为 Diff 和一键回滚 |
| 成本、能耗与资源治理 | 为云模型、GPU、CPU、网络、存储和后台唤醒生成预算、调度、缓存与降级策略 | Resource Policy；硬上限、用户优先级、离线替代、账单预估和禁止以质量名义无限扩容 |
| 联邦与端侧学习 | 基于本地反馈生成个性化适配、评测或可共享的隐私保护更新，不上传原始样本 | Local Adaptation Pack；明确同意、差分隐私或聚合证明、撤回、污染检测和基线回退 |
| 能力退役与数字遗产 | 分析废弃扩展、失效 Provider、孤立数据和无人维护流程，生成导出、替代、归档与安全删除计划 | Retirement Plan；依赖扫描、保留期、可读导出、删除证明和禁止静默丢弃数据 |
...
新增能力不能只成为目录条目。平台按以下顺序把它们转化为可用产品，并允许不同 Provider 在任一步接力：

1. **发现器**：AI 读取用户显式选择且已脱敏的样本，输出机会报告、候选产物和“不应自动化”的理由。
2. **组合器**：优先复用已安装 Command、Event、Query、Connector 和 UI Contribution，生成可视调用图而不是复制代码。
3. **构建器**：缺少能力时才生成 `CreatorArtifact`，同时生成权限、数据流、离线、预算、测试和退役策略。
4. **验证器**：确定性工具负责 Schema、沙箱、回放、属性、性能、无障碍和故障测试；AI 负责解释失败并提出最小 Patch。
5. **经营器**：上线后持续比较预期与实际的调用、成本、延迟、权限使用和用户价值，提出合并、降级、迁移或停用建议。
