...
桌面端通过类型化 `install_asset` IPC 命令调用安装器，WebView 只提交来源目录路径，不再提交 Asset ID 或文件清单；正式 Creator UI 必须由系统目录选择器产生该路径，不能提供自由文本路径输入。Rust 宿主读取 `manifest.json` 获取身份，从 Manifest 指定的完整性文件读取权威清单，并从应用数据目录和合法 namespaced Asset ID 推导目标目录；WebView 不能伪造资产身份、清单或安装目标。每个文件在复制前必须匹配清单中的字节数和小写 SHA-256；单包最多 10,000 个文件、总大小不超过 512 MiB。安全模式会拒绝安装；源目录中的文件即使通过父目录符号链接指向包外，也必须在复制前拒绝。

Creator Studio 已接入 Tauri 官方目录选择器，且权限只授予 `control-center` 窗口。`preview_asset` 使用与安装相同的 Rust 权威校验器，返回身份、类型、版本、发布者、许可证、渲染后端、文件数和总大小；用户明确确认后，`install_asset` 会重新读取并完整复验源目录，再执行原子安装，因此预览后发生的文件变化不能绕过校验。安装与回滚回执只返回包身份和操作结果，不向 WebView 暴露内容仓库路径。当前完成的是安全元数据预览与确认安装，尚未从未安装目录解码海报或模型，不能称为视觉模型预览。

当前实现完成的是**宿主权威目录包导入**。压缩格式 `.nimora` 的限额解包、嵌套压缩包检测、解压倍率限制和独立 Importer 进程仍是后续门禁；在这些能力落地前，UI 只能选择已经展开的目录，不得把任意压缩包直接交给 Core。
