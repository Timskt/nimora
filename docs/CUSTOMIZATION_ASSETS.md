...
桌面端通过类型化 `install_asset` IPC 命令调用安装器，目标目录始终由 Tauri 从应用数据目录和合法的 namespaced asset ID 推导，WebView 不能指定任意安装目标。每个文件在复制前必须匹配清单中的字节数和小写 SHA-256；单包最多 10,000 个文件、总大小不超过 512 MiB。安全模式会拒绝安装；源目录中的文件即使通过父目录符号链接指向包外，也必须在复制前拒绝。
