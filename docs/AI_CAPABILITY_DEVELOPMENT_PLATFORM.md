...
多模态感知、个人 API、Agent Team、策略编译、实验优化、资产流水线、生态迁移等进一步能力及其强制门禁，统一见 [`AI_NATIVE_EXTENSION_SURFACES.md`](AI_NATIVE_EXTENSION_SURFACES.md)。新增能力必须继续归一到本文件的四级扩展模型与 Builder API，不得因为产物类型增加而建立旁路。
...
当前 Creator 已实现 User Program、Skill、Automation、Theme 与核心场景 Profile 的结构化生成、严格检查、摘要绑定批准与受控保存/创建纵切；代码产物使用行为沙箱，Theme 复用 Asset 的固定 Token、WCAG 和完整性门禁，Profile 复用事务化领域服务且创建后不切换。本文定义的是完整目标架构；Connector、Agent Profile、声明式 UI、数据应用生成、多模型接力、仿真包、持续观察、自动修复和退役器仍须逐项形成真实纵切，不能因文档存在而标记完成。
