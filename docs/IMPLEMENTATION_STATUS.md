...
Desktop 组合根治理已开始：Character、Theme、Voice 的选择 Policy、版本化存储、Safe Mode/损坏记录解析和原子写已迁移到 Tauri-free `asset_selection` Application Module，并由机器门禁阻止 Tauri 类型回流。`src-tauri/lib.rs` 仍包含大量 IPC、DTO、Agent、Skill 与资产编排，必须继续拆分，当前不得标记治理完成。
