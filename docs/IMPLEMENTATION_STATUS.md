...
| AI 辅助扩展创作 | 部分实现 | 专用无工具 Draft Agent、System/User 信任分离、严格 JSON 与版本契约、路径/文件预算、权限精确匹配，并复用 User Program、Skill、Automation 生产校验；桌面扩展工作区只展示已验证草案 | 原子 Workspace 写入、独立 Worker 静态检查与沙箱测试、Diff/风险批准、安装启用、运行监控、AI 修复与回滚闭环 |
