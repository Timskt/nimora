...
| AI 辅助扩展创作 | 部分实现 | 专用无工具 Draft Agent、严格 JSON/生产校验、独立 Worker/DryRun、摘要绑定批准与 Workspace 原子保存；Program/Skill 复用生产安装器；Automation 使用版本化 SQLite Catalog，支持命令与行为 Diff、原子安装、强制停用、升级保留上一版、目录启停和回滚 | 多事件/虚拟时间/Mock 行为矩阵、文件/依赖/迁移 Diff、运行期授权策略、真实事件订阅、删除与历史 UI、Connector/Agent/UI/资产生成、监控和 AI 修复闭环 |
