...
| 自动化引擎 | 部分实现 | 独立 `automation-runtime`；版本化 Event Trigger、JSON Pointer Condition、顺序 Action 与 Policy；有界取消/超时；幂等且 retry-safe 的瞬时重试；失败逆序补偿；结构化 Run 结果；原生 IPC 与浏览器同契约零副作用测试运行；自动化工作区 UI；独立 `automation-agent-bridge` 将固定 `agent.task.run` Live Action 经风险、幂等、不可信上下文门禁和统一 AgentTaskGateway 转为共享 Trace/层级预算子任务；Rust/TS 测试 | 桌面 AgentTaskSubmitter 与结果回填、Context Admission/Prompt Injection 检测、并发/冷却/费用门禁；Interval/日历/快捷键等触发器、分支与并行汇合、持久规则和事务运行历史、真实 Capability Gateway Action Backend、虚拟时间与事件回放 |
