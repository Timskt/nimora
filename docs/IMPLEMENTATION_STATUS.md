...
## 2026-07-24 — lifeformPerf / processBudget / 通知感官 / Grant 加密 / 跨屏与 Q-minion 抛光（本切片）

### DONE

- **lifeformPerf 产品化**
  - `apps/desktop/src/components/lifeformPerf.ts`：滚动窗口帧采样、`LIFEFORM_PERF_BUDGET`（目标 ~60fps / 告警帧时 / 内存 200MiB / 空闲 CPU ~2%）、`nimora:lifeform-perf` CustomEvent（~500ms 节流）。
  - `BuiltinPet3D` 每帧 `recordFrame` → 汇总 emit。
  - Control Center `LifeformOverview` **「渲染预算」** 卡（FPS / 帧时 / 可选 Host RSS·CPU tone）。
- **processBudget IPC on `DesktopSnapshot`**
  - Host `process_budget::sample_process_budget` 挂入 snapshot / lifeformSense 路径：`rssBytes` / `rssMb` / `cpuPercentApprox` / `withinMemoryBudget` / `observedAtMs`。
  - **macOS**：`task_info(MACH_TASK_BASIC_INFO)` 优先；fallback `getrusage`。
  - **Linux / 其他 Unix**：`/proc/self/status` `VmRSS` 优先；fallback `getrusage`（KiB）。
  - **Windows**：`GetProcessMemoryInfo` → `WorkingSetSize`（零则 `PrivateUsage`）；API 失败 fail-closed。
  - FE 防御性读取：`hostProcessBudgetFromSense` 接受扁平字段或嵌套 `processBudget`。
- **notification unread sensory（隐私安全）**
  - 仅用 outbox / approvals 等**计数**边沿（`notification_unread_from_counts` / `notification_should_emit`），**不下发**标题/正文/发件人。
  - Domain：`notification_sensory_directive(has_unread)` → Host 边沿 `apply_lifeform_directive_from_host`。
  - `LifeformSense.notification_unread: bool` 进入 snapshot 投影。
- **Grant at-rest crypto API**
  - `AuthorizationGrantKey`（256-bit；`generate` / `from_hex` / `app_local_default`；Debug redacted）。
  - Envelope：`nimora.encrypted-authorization-grant/1`，**XChaCha20-Poly1305** seal/open。
  - **Dual-read**：legacy plaintext payload 仍可加载；**写路径加密**（issue/revoke 再写 envelope）。
  - Host 已接线：`authorization_grant_key` 从 OS secret-store 加载/生成（`secret:cache:authorization-grant-v1`），`open_authorization_grant_repository` 用于 issue/list/revoke/control-center/auto-mode；secret store 不可用时 fallback `app_local_default`（测试/无钥匙串环境）。
- **跨屏漫游抛光**
  - `plan_cross_display_target`：`sequence % 5` **五档 park slot**（左→右 bottom ground），避免总停同一底中心。
  - `plan_lifeform_wander_goal`：**每隔一次** wander（`sequence % 2 == 0`）优先跨屏目标；短距偶发 `MotionMode::Jump`（抛物线帧，非 lerp）。
  - 多屏 spring bounds 继续用 union work area。
- **Q-minion 视觉抛光**
  - 无方框 chrome / 透明 canvas；`.pet-hit-area` **ellipse clip-path** 命中。
  - 更活 idle（yawn / dig-nose / count-ants / look-around / micro-bounce）。
  - **双层接触阴影**（宽 soft pool + denser core）。

### 门禁（本切片本地 · 已跑）

- FE components **235** · desktop-context **45** · runtime-core behavior **30** · grant encrypt suite **10** · process_budget **7** · desktop_lifeform **18**

### 仍须原生验收（不得标 goal complete）

- `pnpm tauri:dev` 真机：透明无框、拖拽、穿透、遮挡、跨屏走/跳手感、渲染预算 HUD 与真实采样一致。
- Idle CPU/内存预算**真机实测**（插桩产品化 ≠ 预算达标证明）。
- Windows 签名包；Windows processBudget 真机验收（代码已接 GetProcessMemoryInfo）。
- Grant：secret-store 生产密钥轮换 / 设备绑定策略产品化仍开放（API 已落地 ≠ 安全闭环宣称）。

---
## 2026-07-24 — User-code Subject 控制 + 微表演 + Agent 上下文面（本切片）

### DONE
- **safe.pet.directive**：用户程序 / Skill 经 Capability Gateway 提交 `nimora.pet_directive/1`（speech / moodDelta / action / animation / attention）；目录 `pet.action.catalog` 暴露完整 directive 契约。
- **微表演白名单**：`pet.yawn` / `pet.dig_nose` / `pet.count_ants` / `pet.wave` / `pet.look_around` / `pet.hop`；BuiltinPet3D 定向放大 idle 通道（无线性位移）。
- **通知感官**：`notification_sensory_directive`；`request_attention` Notification 通道批准后宠物看向通知区。
- **Agent UX**：Goal 控制卡「上下文」条（token/压缩/缓存）、「工作区追踪」条、思考档位 chip。
- **Creator 能力矩阵**：中文 chips + `safe.pet.directive` 示例 plan；`docs/USER_CODE_CAPABILITY.md`。
- **lifeformSense** 与 Q-minion 接地表现沿用上一切片。

### 门禁（本切片本地）
- desktop lib **252** · runtime-core behavior **30** · FE components **224** · `tsc -b` clean

### 仍须原生验收（不得标 goal complete）
- `pnpm tauri:dev` 真机：透明无框、拖拽、穿透、遮挡、directive 驱动、用户程序 directive。
- Idle CPU/内存预算实测；跨屏手感；Windows 签名包。（Grant at-rest crypto API 见本文件顶部 2026-07-24 切片。）

## 2026-07-24 — Control Center `lifeformSense` IPC + Q-minion 接地抛光（本切片）

### DONE
- **lifeformSense IPC（DLC-PET-10）**：`desktop_snapshot` 经 `lifeform_sense_from_env` 暴露隐私安全聚合（无窗口标题 / 前景正文 / 像素）。
- **聚合字段**：`batteryPercent` / `onBattery` / `charging`；`idleMs`；`meetingActive` / `meetingHint`（仅 `zoom|teams|meet|webex|unknown`）；`displayCount`；`freshness` / `degradationReason`；`observedAtMs` / `expiresAtMs`。
- **Host 刷新**：lifeform 采样后 emit `system-context-changed`，驱动控制中心权威 snapshot 刷新。
- **FE 轮询合并**：`LifeformOverview` 每 5s 始终 poll + merge `lifeformSense`（`buildLifeformSenseHintsFromSnapshot` / `applyLifeformSenseSnapshot`）。
- **接地阴影**：`CONTACT_SHADOW_STOPS` 更密，贴地感与测试对齐。
- **Q-minion 服装**：`BuiltinPet3D` 牛仔背带裤（肩带 + 口袋）。
- **测试**：Host `lifeform_sense_from_env_projects_privacy_safe_aggregates`；FE 投影覆盖电量/空闲/会议/多屏 + 阴影/服装相关对齐。

### 门禁（本切片本地 · 当前）
- `cargo test -p nimora-desktop --lib` → **250** passed
- `cargo test -p nimora-runtime-core --lib behavior` → **27** passed
- `cargo test -p nimora-desktop-context --lib` → **43** passed
- FE：`pnpm exec vitest run src/components` → **202** passed（21 files）
- `tsc -b` clean
- 功能计划：`DLC-PET-10`（Control Center lifeformSense IPC）**已落地**

### 仍须原生验收（不得标 goal complete）
- `pnpm tauri:dev` 真机：透明无框、拖拽、穿透、遮挡、会议/电量/空闲宠物化与 Control Center 感官卡与真实采样一致。
- Idle CPU/内存预算实测；跨屏走过去手感；Windows 签名包。（Grant at-rest crypto API 见本文件顶部 2026-07-24 切片。）

---
## 2026-07-24 — Host lifeform 感官闭环 + Q-minion FE 抛光（本切片）

### DONE
- **Meeting sensory 接线**：OS 采样边缘触发 `meeting_sensory_directive`（Zoom/Teams/Meet/Webex 中文安静陪同；会议结束 soft observe）；`runtime-core` 正式 re-export。
- **Grant petization 统一**：`unattended_auto_mode` issue/revoke 走 `companion_directive::apply_grant_event`。
- **位置持久化**：`move_pet` / `finish_pet_drag` 调用 debounced `schedule_position_persistence`；overlay 模式下 logical pet 仍为源真相。
- **编译修复**：`unattended_auto_mode` 测试 `BTreeSet<SandboxScope>` → `Vec`（`SandboxScope: !Ord`）。
- **Q-minion FE**：暖黄双目护目镜、背带裤/圆靴、双层 contact shadow、idle yawn/dig-nose/count-ants/look-around；透明 canvas 无方框；身体形 hit area + 中文软气泡。
- **Stage/body**：全屏透明 AOT overlay + 260×300 逻辑体；Subject 规则 `directive > companion > lifecycle`。
- **跨屏漫游**：`execute_pet_wander` 在多显示器时使用 `spring_position_frames_multi_display`（union bounds，不夹死在单屏）。

### 门禁（本切片本地）
- `nimora-desktop --lib` **248** passed（含 multi-display spring wander 接线）
- `nimora-runtime-core` behavior **27**
- `nimora-desktop-context` **37**
- FE vitest pet/control 切片 **147** · `tsc -b` clean
- 功能计划：`DLC-PET-07/08/09`（会议安静 / grant 宠物化 / 位置 debounce）

### 仍须原生验收（不得标 goal complete）
- `pnpm tauri:dev` 真机：透明无框、拖拽、穿透、遮挡、会议/电量/空闲宠物化、Agent/Auto→directive。
- Idle CPU/内存预算实测；跨屏走过去手感；Windows 签名包；Grant at-rest crypto。

---
## 2026-07-24 — Lifeform Subject 闭环强化（Q-minion 指令驱动 + Away/Grant 深度）（本切片）

### DONE
- **Directive → 3D 动作**：`PetOverlay` Subject 规则优先 `directive.animation/action`；`BuiltinPet3D.normalizeMotionState` 对齐 `work_busy` / `work_crash` / `pet.*` 词汇；Agent 干活→出汗、崩溃→晕眩。
- **Q-minion 表现**：双目不对称眨眼、cheek flush、hair/antenna spring 次级运动、双层接触阴影、overalls/goggle 强化；透明 stage 无方框；260×300 body + 多屏 local clamp。
- **Host petization**：`CompanionPhaseTracker` StepOk 每 3 批节流（可重复观察）；Grant issue/revoke → 宠物化 speech；Away Summary 丰富 highlights（预算/确认暂停、批次用量、高风险 grant）。
- **控制中心**：五档 Grant 徽章、NeverAskWithinGrant 睡眠安全文案、Away Summary 中文态、LifeformOverview Subject 叙事（Skill/Worker/Connector）绑定 snapshot。
- **Unattended**：`sha256:`+64hex fingerprint 校验、tier 策略、睡眠安全 NeverAsk。

### 门禁（本切片本地）
- desktop lib away/companion/unattended **41** · runtime-core behavior **25** · FE vitest pet/control **107** · `tsc -b` clean

### 仍须原生验收（不得标 goal complete）
- `pnpm tauri:dev` 真机：透明无框、拖拽、穿透、遮挡、Agent/Auto/Automation→宠物指令、电量/空闲宠物化。
- Idle CPU/内存预算实测；跨屏走过去手感；Windows 签名包；Grant at-rest crypto。

---
## 2026-07-24 — Automation petization host + Auto Mode domain events（本切片）

### DONE
- **Automation 宠物化**：live automation 生命周期接入 `automation_directive` / `companion_directive::apply_automation_phase`：
  - `Triggered` 于 admitted run 启动
  - `Succeeded` / `Failed` 于终端状态
  - `Throttled` 于 cooldown / concurrency 拒绝
- **Auto Mode 领域事件**：`CompanionPhaseTracker` 支持 `Started` / `StepPaused` / `Completed` / `Budget` / `Crashed`，并在批处理成功路径 **节流 StepOk**（每 3 步）。
- **Grant 委托 domain**：`grant_event_directive` 以 `grant_directive` 为底，叠加五档中文 speech / body-language。
- **控制中心 UX**：Away Summary 中文态、无人值守五档风险说明、LifeformOverview Subject 叙事（Skill/Worker/Connector）。
- **默认 3D**：BuiltinPet3D 双目 Q-minion 优先链；透明 stage；无方框 canvas。

### 门禁（本切片）
- companion_directive **18** · runtime-core behavior **25** · FE vitest pet/control **85** · tsc clean

### 仍须原生验收（不得标 goal complete）
- `pnpm tauri:dev` 真机：透明无框、拖拽、穿透、遮挡、Agent/Auto/Automation→宠物指令、电量/空闲宠物化。
- Idle CPU/内存预算实测；跨屏手感；Windows 签名包。


## 2026-07-24 — 文档诚实性 + lifeform/unattended 闭环（本会话关闭项）

本会话**只闭合文档与测试计划中的过时“剩余项”声明**，与已落地代码对齐；**不**新增原生视觉证明。

### 本会话关闭（相对旧切片“剩余”列表）
- **Away Summary 产品面**：桌面 `away_summary` + IPC `get_away_summary` + FE `AwaySummaryPanel` / `getAwaySummary` + CLI `nimora ai goal auto away-summary` — 代码与本地门禁已有；旧文中“产品面剩余”作废。
- **CLI Grant 管理面**：`nimora ai goal grant issue|list|show|revoke` + 集成测试 — 已有；旧文“CLI 管理面剩余”作废。
- **Domain petization factories 清单对齐文档**：`agent_status` / `auto_mode` / `user_code` / `automation` / `battery` / `idle_user` / `grant`（+ Host 侧 `connector_sensory` / skill worker 路径）与 stage/body **260×300** 架构写入 `DESKTOP_LIFEFORM_CONTEXT.md`。
- **NeverAskWithinGrant + full_device 风险 + sleep-safe** 写清于 `AUTO_AUTHORIZATION.md`；功能测试补齐 lifeform / away-summary / grant 用例编号。

### 仍明确未证明（禁止 goal complete / production-ready 宣称）
- **原生 visual QA**（透明、穿透、遮挡、多屏手感）
- **Signed Windows package**
- **Idle CPU / 内存预算实测**
- Grant **at-rest 系统密钥签名/加密**；独立 **Auto-review** 流水线；**Full Device 真机危险矩阵**；组织/OS policy 交集

### 门禁引用（既有，非本会话重跑全部）
- CLI：`authorization_grant_cli_*` + `auto_mode_away_summary_*`；desktop `away_summary` 单元；runtime-core petization；详见下方同日切片。

---

## 2026-07-24 — Domain petization + Away Summary + dual-eye Q-minion（本切片）

### DONE
- **runtime-core petization factories**：`agent_status_directive` / `auto_mode_directive` / `user_code_directive` / `automation_directive` / `battery_directive` / `idle_user_directive` / `grant_directive`（mood 使用 `i8`，非 `Option`）；单元测试校验通过。
- **Host 宠物化接线**：User Program → `user_code_directive`；OS sensory 电量/空闲阈值节流 → `battery_directive` / `idle_user_directive`；connector 相位仍用 `connector_sensory_directive`；`companion_phase_directive` 委托 domain `agent_status_directive` 保持 FE 同 token。
- **Away Summary E2E**：`apps/desktop/src-tauri/src/away_summary.rs` + `get_away_summary` IPC；FE `desktopApi.getAwaySummary` + AgentWorkspace 面板；CLI `nimora ai goal auto away-summary` + grant issue/list/show/revoke。
- **Persistence**：`SqliteAutoModeRepository::list_for_goal` 供 Away Summary / 历史会话发现。
- **默认 3D**：`BuiltinPet3D` 双目 Q-minion 优先链；idle micro-acts（yawn/digNose/countAnts/microBounce）。

### 门禁（本切片本地）
- runtime-core behavior petization **25** · desktop companion **13** · away_summary **3** · FE vitest 相关 **71** · tsc clean

### 仍须原生验收（不得标 goal complete）
- `pnpm tauri:dev` 真机：透明无框、拖拽、穿透、遮挡、Agent→宠物指令、电量/空闲宠物化。
- Idle CPU/内存预算实测；跨屏走过去手感；Windows 签名包。



## 2026-07-24 — Unattended Grant + Multi-monitor displays + FE spring/unattended UX（DONE 切片）

### DONE — Unattended grants
- **领域**：`AuthorizationGrant`（`nimora.authorization-grant/1`）精确绑定 Goal/Plan/Workspace/Tool/Provider/Model/数据等级/网络；`ApprovalPolicy::NeverAskWithinGrant` 在有效 Grant 内返回 `Authorized`（sleep-safe：用户离开时范围内不弹确认，仍受预算/绑定/硬禁区约束）。
- **Auto Mode**：`evaluate_step_with_grant` + `AutoModeTurnSupervisor::with_authorization_grant`；过期/撤销/越界仍 pause。
- **SQLite**：`SqliteAuthorizationGrantRepository` — `issue` / `get` / `get_active_for_goal` / `revoke` / list。
- **桌面 Host**：`unattended_auto_mode` 五档 `tier_policy`；`start_unattended_auto_mode` 原子创建 Goal+Session+Checkpoint+Workspace+Grant+Job；`list`/`revoke` IPC。
- **Runner**：每批 `get_active_for_goal` 注入 Supervisor。
- **Schema**：`@nimora/schemas` — grant 完整/摘要、`startUnattended*`、`nimora.pet_directive/1`。

### DONE — Multi-monitor display enumeration
- **`current_lifeform_displays`**：`app.available_monitors()`（fallback 到 pet/control window monitors）枚举 **全部** 已连接显示器；id/几何/work_area/scale_factor 去重写入 `DesktopDisplay[]`。
- 用于：`overlay_stage_for_pet` mid-walk rebind、`pet_crosses_display_boundary`、`plan_cross_display_wander_target` 跨屏漫游目标。
- **不得再写**“只取宠物当前显示器 / 完整多屏 displays 图未做”——Host 枚举已落地。跨屏走过去的 **手感打磨** 与真机多 DPI 验收仍为剩余体验项。

### DONE — FE spring + unattended UX
- **BuiltinPet3D**：secondary-motion **spring-damper**（`springToward` / 半隐式 Euler；非线性 lerp）驱动 scale/tilt/limbs/sweat 等通道。
- **AgentWorkspace**：五档中文标签、`tierApprovalPolicy` / `tierSandboxLabel` / `tierRequiresDangerAck`、grant badge 状态色、`startUnattendedAutoMode` + list/revoke 控制面。
- **Companion**：`agentCompanionDirective` ↔ host `companion_directive` 同 token（work/perch/celebrate/rest）。

### 门禁（占位，parent 已验证绿）
- auto-host **21** · grant store **6** · unattended unit **7** · runtime auto **13** · FE vitest **58**

### 剩余（诚实标准，非 defer）
- Grant at-rest 系统密钥签名/加密；独立 Auto-review；Full Device 真机危险矩阵（见 `AUTO_AUTHORIZATION.md` §8.3）。
- **Away Summary 产品面 / CLI grant 管理面：已落地**（见上方本会话切片与 Domain petization 切片；勿再标剩余）。
- 原生 visual QA · signed Windows package · idle CPU 预算测量；跨屏 walk **体验**打磨、遮挡像素精修（代码路径已有，真机未证）。

## 2026-07-24 — Lifeform Subject slice (overlay + agent directive + multi-mon + petization)

- **Overlay stage**: transparent AOT stage = monitor work area; logical pet body **260×300** (body is **not** the OS window); spring-damper motion only; hit-test ignore-cursor except body; multi-monitor **mid-walk** stage rebind + cross-display wander preference; **`current_lifeform_displays` full monitor enumeration DONE**.
- **FE**: BuiltinPet3D secondary **spring-damper** (`springToward`) + AgentWorkspace unattended grant UX (tiers/danger ack/badges) **DONE**.
- **Agent → Subject**: AgentWorkspace publishes companion signal **and** `applyPetDirective` (`nimora.pet_directive/1`); FE `applyPetDirective` types fixed.
- **Auto Mode → Subject**: host `auto_mode_runner` + `companion_directive` map job status → structured directives with phase de-dupe (`work` / `wait` / `complete` / `fail` / `cancel` → work / perch confirm / celebrate / rest / work_crash).
- **Skill/Worker petization**: `skill_worker_busy_directive` / `skill_worker_done_directive` applied on execute + approve paths; approval wait → perch confirmation phase.
- **OS sensory**: lifeform sample transitions (offline / degraded / low battery / restored) → sensory directives (phase-tracked, no spam).
- **Connector EventReceived**: `connector.*` events emit petized `EventReceived` via automation/skill event sessions; when `AppHandle` present, admitted automation path applies directive (**throttled 8s**).
- **Control Center**: `LifeformOverview` — lazy `BuiltinPet3D`, vitals, personality, directive summary, Chinese attention labels, overlay stage summary; soft WebGL fallback; live `onPetDirectiveChanged`.
- **Schema**: optional `personality` / `lastDirective*` / `lastAttention` / `directiveRevision` on pet schema; preview snapshot enriched with lifeform fields.
- **Renderer polish**: BuiltinPet3D idle micro-acts `yawn` / `digNose` / `countAnts`; Q-minion warm yellow palette.
- **Gates (local)**: `desktop_lifeform` **11** tests, FE related **~75** tests, `tsc` clean — green. Broader suite status unchanged unless re-run.
- **Not proven**: native visual QA, signed Windows package, idle CPU budget measurement. Do **not** claim lifeform goal complete or production-ready visual proof.

## 2026-07-24 — Fullscreen Overlay Subject + Occlusion + Lifeform Intent Host

- **Stage ≠ body**：透明全屏 work-area Overlay；逻辑宠物体 `260×300`；`Pet.position` = 屏幕物理 top-left；前端 `--pet-local-x/y` 放置 `.pet-subject`。
- **运动**：弹簧帧 emit `nimora://pet-position-changed`；逻辑拖拽（`begin_pet_drag` / `move_pet` / `finish_pet_drag`）；命中测试 click-through（仅宠物体可点）。
- **遮挡纯库 + Host**：`nimora-desktop-context::occlusion` + `pet_occlusion_for_pose`；Host 采样后发 `nimora://pet-occlusion-changed`；前端 clip-path / ambient mute。
- **多屏**：`overlay_stage_for_pet` / `pet_crosses_display_boundary`；Host 在采样、move、wander 结束时 rebind stage。
- **结构化意图**：`Pet::apply_structured_directive`、`tick_autonomy_with_lifeform`；Host 自主 Tick 接入 `DesktopBehaviorHints`；IPC `apply_pet_directive` + 事件 `nimora://pet-directive-changed`。
- **门禁（本切片）**：desktop-context **22**；desktop_lifeform **18**；runtime-core **77**；runtime-app **41**；desktop lib **201**；frontend vitest **131**。
- **仍开放**：真实像素级遮挡合成精修、完整跨屏“走过去”体验打磨、原生视觉 QA、perf / idle CPU 插桩、Windows 签名包样本验收。（Agent/Auto Mode/Skill/OS/Connector → structured directive **代码路径已接线**，不得用本条冒充原生验收完成。）


## 2026-07-24 — 桌面生命体情境 / 弹簧运动 / 人格指令纵切

- 平台无关 crate **`nimora-desktop-context`**（`crates/desktop-context`）：`DesktopSnapshot`（`nimora.desktop-context/1`）、新鲜度 fail-closed、弹簧-阻尼（`integrate` / `integrate_to_target`，**非**位置 lerp）、障碍规划、`plan_wander` / `plan_wander_from_snapshot`。crate 内约 **15** 个单元测试。
- macOS 采样 **`nimora-desktop-context-macos`**：有界超时 `sample()`；CGWindow 几何（默认不存标题）、idle、会议进程名启发；电源经 IOKit `IOPSCopyPowerSources*`，失败 → `PowerFact::unavailable()`（fail-closed，不是“永远不可用”，也不是电量产品闭环完成）。
- Windows 采样 **`nimora-desktop-context-windows`**：`cfg(windows)` Win32 窗口/idle/电源/会议；非 Windows → `AdapterUnavailable`。**不等于** Presence 栈的 `system-context-windows`。
- **`runtime-core` 行为域**：`PersonalityProfile`（挂 `Pet` 快照）、`MoodAxis`、`StructuredPetDirective`（`nimora.pet_directive/1`）、`DesktopBehaviorHints`、确定性 `select_autonomous_intent` → `into_directive()`。宠物在契约上为 Subject；Host 经 `tick_autonomy_with_lifeform` 接入 lifeform hints。
- **Desktop Host 接线**（`desktop_lifeform` + `lib.rs`）：
  - path 依赖 desktop-context 与平台采样 crate；`lifeform_env` 缓存最新快照。
  - 主循环约每 5s、`sample(..., 1.5s)`；失败写入 Degraded 快照；OS 感官 + connector `EventReceived`（8s 节流）可 petize。
  - Overlay stage + 逻辑宠物体 260×300；`plan_lifeform_wander_goal` → `spring_position_frames` → **逻辑位姿 emit**（非 body-as-window）；多屏 mid-walk rebind。
  - 边缘 Surface 目标仍用 `plan_surface_wander_target`，再套弹簧帧。
  - 会议 `active` 时粗粒度设置 `presence_decision.suppress_autonomy`（非正式 Presence 信号模型）。
- Frontend：透明 AOT overlay + 逻辑 `.pet-subject`；`BuiltinPet3D` idle micro-acts（yawn / digNose / countAnts）；Control Center `LifeformOverview`；Fox GLB 接地取景路径仍保留为角色加载链。
- Host 模块：`mod desktop_lifeform` 提供映射、遮挡发布、stage rebind、directive 应用与本地测试（**11**）。

**集成诚实边界（勿标绿）**

- Overlay stage + 逻辑宠物体架构 **已落地**（stage ≠ body）；真机透明/拖拽/穿透手感仍须原生视觉 QA，**不可**用 Browser Preview 冒充。
- 真实窗口像素遮挡渲染精修：**未完成**（库级 occlusion + clip-path 已有；像素级合成打磨开放）。
- Agent / Auto Mode / Skill / OS / Connector → structured directive 产品路径 **已接线**；原生端到端 UI 闭环仍须 Tauri 真机验收。
- `select_autonomous_intent` 已接入 lifeform Host Tick 路径；人格偏置产品化与边缘 Surface 统一规划仍开放。
- `current_lifeform_displays`：**DONE** — 枚举全部 `available_monitors`（work area/scale）；跨屏 wander/rebind 可消费全图。跨屏 walk 手感与混合 DPI 真机验收仍开放。
- 性能预算端到端插桩（采样 P95、积分帧时、WebView GPU、idle CPU）：**未做 / 未测**。
- Windows 全量环境样本路径存在，签名包真机质量与权限矩阵仍须验收，不得用 macOS 结果冒充。

证据：desktop-context / macos / windows 单元测试；`desktop_lifeform` **11**；FE related ~75；`tsc` clean；Host 含采样、弹簧逻辑位姿、directive / sensory / connector EventReceived 路径。本轮**不**声称原生多屏视觉、遮挡像素合成精修、signed Windows 包或 idle CPU 预算已通过。细节见 [`DESKTOP_LIFEFORM_CONTEXT.md`](DESKTOP_LIFEFORM_CONTEXT.md)；与 Presence（`system-context`）职责分离。
...
- 活动工作区不再停留在应用启动时的一次性 Outbox Snapshot；进入活动页立即刷新，前台可见期间每 10 秒读取一次权威本地快照。
- 页面隐藏时不读取数据库，恢复可见后立即刷新；离开活动页清理定时器和监听器，慢请求通过单次飞行保护避免重入。
- 刷新失败保留上次成功结果并给出非破坏性提示，不清空计数、不新增网络遥测，也不把正文、Prompt、文件路径或桌面内容带入 UI。
- 已验证：Frontend 106 项测试与 TypeScript 通过；Chrome 810px 实测刷新说明、活动导航、零横向溢出与零 Console Error 通过。

## 2026-07-19 — 控制中心活动与设置导航闭环

- 修复两个可见死按钮：概览“查看全部”现进入独立活动工作区，顶栏头像进入设置；一级“活动”不再错误回落并重复渲染概览。
- 活动工作区复用现有权威 SQLite Outbox Snapshot，展示待投递、租约中、已投递、Dead Letter 和核心本地健康摘要；不创建第二套日志、不读取事件正文、Prompt、文件路径或桌面内容，也不依赖网络。
- 一级导航当前项新增 `aria-current=page`；活动页提供明确标题、健康非纯颜色文字、语义区域、响应式四/二/一列摘要。浏览器实测三个入口、810px 零横向溢出、可访问树和零 Console Error通过。

## 2026-07-19 — 控制中心窄宽度与 200% 重排基础

- 移除控制中心页面级 `720px` 最小宽度锁；在 720px 及以下将固定侧栏重排为粘性顶部导航，品牌、本地运行状态和全部一级入口继续可见，导航支持横向滚动而非裁切。
- 工作区、顶栏与概览 Hero 进入单列，480px 以下全局操作和 Hero 操作按语义换行；不通过缩小字号或隐藏关键功能伪装适配。Pet Window 既有 260×300 独立几何不受影响。
- 当前浏览器插件固定为 810px，已验证现有宽度零横向溢出、零 Console Error 和视觉无回归；插件不能调整视口或缩放，因此 720/480px 与真实 200% 仍诚实保留为可控浏览器/签名桌面门禁，不用 CSS 静态存在冒充截图通过。

## 2026-07-19 — 桌宠完整菜单键盘等价入口

- QQ 宠物式 Overlay 的主互动按钮现支持标准 `ContextMenu` 键与 `Shift+F10`，键盘和辅助技术用户无需模拟鼠标右键或长按即可打开完整径向菜单。
- 快捷键只在桌宠主按钮处理，阻止浏览器/系统默认菜单后复用既有 `openPetMenu`、首项聚焦、方向键/Home/End 导航与 Esc 焦点归还链路；Enter 和 Space 继续保持普通互动语义。
- 快捷键判定抽成无 DOM 纯函数，单元测试覆盖标准菜单键、组合键与非目标按键；浏览器实测确认菜单展开语义、首项焦点、方向键移动和 Esc 关闭。透明、置顶与原生系统菜单冲突仍由签名 Tauri 包做最终门禁。

## 2026-07-19 — Pet WebView 心跳自愈

- Pet Overlay 每 15 秒通过仅限 Pet Window 的 Typed Port 上报无内容心跳；Tauri Host 以 90 秒阈值检测“窗口仍在但 WebView 已卡死”，前端不能请求重建或重置恢复预算。
- 看门狗在窗口隐藏时暂停，Presentation/Profile 隐藏、系统睡眠和后台节流不会消耗恢复次数；重新可见先重新武装完整宽限期。
- 失联窗口由 Desktop Coordinator 销毁并复用既有每分钟三次指数退避恢复；Destroyed 事件、看门狗和托盘恢复共享单 Worker 门禁，shutdown intent 发布后永久拒绝复活。
- 心跳完全离线、不写数据库、不记录页面状态或用户内容；单调 `fetch_max` 防止时钟回拨或迟到心跳把存活时间倒退。
- Rust 测试覆盖阈值边界、回拨、隐藏暂停、重新武装、恢复预算与退出竞态；Frontend 精确 IPC、TypeScript、Clippy 和架构边界通过。真实 WebKit/WebView2 进程终止仍由签名原生包故障注入门禁验证。
...
- 可用令牌与下一补充时间随 Pet 快照持久化，退出、重启、窗口隐藏和宿主循环重建均不重置；旧快照缺字段时安全初始化，系统时钟回拨不增发。
- 预算耗尽只将下一自主动作推迟到最近补充点；手动互动、照料、拖拽、生命值和离线恢复不受影响。即使外部调用方直接伪造 `Start`，Core 也拒绝透支且不改变动作或序列。
- 聚焦测试覆盖两次耗尽、平滑补充、序列化恢复、旧快照迁移、时钟回拨、伪造 Start 与异常容量钳制；真实 24 小时观感仍属于签名桌面长稳门禁。
...
- 当前自动化证据覆盖确定性选择、动作结束、冷却、用户抢占、安静时段设置 UI、持久频率预算、负坐标副屏、四向边缘夹取、完全离屏主屏恢复、最大交集屏幕选择、分辨率缩小和显示器枚举缺失；真实热拔插观感、可用工作区/系统 Dock 感知与 24 小时真机长稳仍按发布门禁继续验收。
...
|---|---|---|---|
| 桌宠窗口与交互 | 部分实现 | Tauri 透明双窗口、拖拽、置顶、穿透、托盘、安全模式、Click/Drag FSM、可访问六向径向菜单、安全深链、Agent 六态陪伴反馈与任务协作成长、经典互动、低压力 Needs、背包、纪念、身份、家位置、持久自主频率预算、宿主全局注意力预算与 WebView 心跳自愈；Core 权威派生关系阶段；Desktop Coordinator 统一 Profile、系统情境、Safe Mode、用户覆盖、主动呈现与有界窗口恢复 | 完成 macOS 屏幕共享/游戏 Adapter、更多角色响应、多屏/DPI、签名包 WebView 故障注入与 8 小时长稳验证 |
...
Agent Runtime 已新增 Provider 无关的推理策略领域契约，统一 `auto/minimal/low/medium/high/very_high/maximum` 语义和 Adaptive/Quality First/Cost Saver/Fixed 策略；显式不支持等级 fail-closed，并可审计 requested、actual 与 Provider value。范围绑定 `AuthorizationGrant` 已覆盖 Sandbox、Approval、Network、寿命、Goal/Plan/Workspace、Tool、Provider、模型、数据等级和预算，支持过期、撤销、绑定漂移与范围外拒绝。**SQLite Grant 仓储、Auto Mode 每批 Grant 准入、桌面 `start_unattended_auto_mode` / list / revoke 与 AgentWorkspace 授权 UI 已落地**（`NeverAskWithinGrant` sleep-safe）。仍为剩余项：系统密钥保护/签名、独立 Auto-review、Provider capability/mapping 与 Context Cache 推理指纹的完整对齐。**Away Summary 产品面与 CLI grant 管理面已落地**（见 2026-07-24 本会话切片）。原生 visual QA / 签名包 / idle CPU **未证明**。
...
## 2026-07-19 QQ 宠物式 Overlay 交互审计

- 已确认：桌宠以独立 `260×300` Overlay 呈现，角色是主要命中目标；悬停显示低打扰快捷工具条，移开后自动收起，不长期占据桌面。
- 已确认：右键与 `Shift+F10` 打开同一环形菜单，键盘入口自动聚焦首项；主按钮及快捷动作均有可见焦点与完整无障碍名称。
- 已确认：浏览器 Preview 在 `810×639` 下无页面级横向或纵向溢出，悬停气泡、工具条和环形菜单不裁切角色面部，Console 无 Warning/Error。
- 产品边界：QQ 宠物式体验还包括 Tauri 透明置顶窗口、桌面拖拽、点击穿透、托盘召回、Work Area 夹回、WebView 自愈、离线照料与自主行为；这些能力由 Desktop Coordinator 执行，Overlay、AI、Renderer、Skill 和用户程序不能直接控制原生窗口。
- 证据边界：本轮 Chrome 截图只证明 Web 内容的排版、焦点与菜单行为，不证明透明合成、原生置顶、跨显示器拖拽、DPI 或任务栏/Dock Work Area。
