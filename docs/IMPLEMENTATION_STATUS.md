...
| AI Agent 与 CLI | 部分实现 | `nimora.agent-tool/1`、Tool Registry、参数风险提升、调用批准指纹、模块 Backend 边界、统一任务状态机及步骤/工具/时间/Token/费用硬预算与 Rust 测试 | Provider Adapter、真实 Capability Gateway 接线、计划循环、桌面/CLI、历史、离线降级、Safe Mode 强停与 Prompt Injection 防护 |
