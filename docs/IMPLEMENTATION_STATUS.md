...
|---|---|---|---|
| 桌宠窗口与交互 | 部分实现 | Tauri 透明双窗口、拖拽、置顶、穿透、托盘、安全模式、Click/Drag FSM、经典单击/双击/长按/右键/抚摸仲裁、四项低压力 Needs、背包道具、纪念收藏、持久身份与家位置；原生窗口标题随持久名称启动并以可逆事务同步改名；Affinity/BondPoints、旧快照迁移、SQLite 原子持久化与真实关系卡均有 Rust/TS 测试 | 真正径向布局、更多抚摸区域/角色响应映射、任务协作成长、显式关系阶段、环境降扰、Windows/macOS 多屏/DPI、WebView 崩溃恢复与 8 小时长稳验证 |
