...
| 扩展与 Skill 生态 | 部分实现 | 规格与用户代码基础设施；独立 `skill-runtime` 实现 `nimora.skill/1` 严格 Manifest、命名空间 Command/Agent Task Contribution、精确版本与 Capability 授权、激活快照、暂停撤销、显式崩溃恢复、五分钟三次崩溃 quarantine、用户解除隔离、非活跃卸载及仅激活期发放 `skill:<id>` AI requester | 事务包安装、完整性与签名、独立 Skill Worker/Supervisor、IPC 与资源配额、Command Registry 接线、Desktop Module Submitter、事件/UI/Tool 等 Contribution、管理 UI、官方番茄钟与提醒 |
