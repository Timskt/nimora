...
| AI 辅助扩展创作 | 部分实现 | 专用无工具 Draft Agent、System/User 信任分离、严格 JSON 与版本契约、生产校验；桌面可将复验后的草案显式保存到 Workspace 隐藏目录，写入拒绝覆盖/符号链接并以 staging rename 原子发布 | 独立 Worker 静态检查与沙箱测试、Diff/风险批准、安装启用、运行监控、AI 修复与回滚闭环 |
