...
| AI Agent 与 CLI | 部分实现 | Tool Registry、风险与批准、任务硬预算；Provider/Tool 单步协调；四项生产模块工具及共享 Capability Gateway 固定映射；桌面 Agent 工作台可查看生产目录并运行零费用确定性离线诊断任务；CLI 工具发现；`nimora` 离线 CLI；独立 sidecar 中真实 Ollama `/api/chat` Adapter；loopback-only SSRF 边界；Manifest 与宿主可信摘要驱动的 Worker 完整性发现；CLI Ollama 接线；有界协议、超时取消强杀、Tool Call 转换及跨进程 mock 集成测试 | 桌面 Tool Call 参数确认与执行、桌面 Ollama 自动发现、扩展生产工具覆盖面、OpenAI-compatible Adapter、发布者数字签名、持久计划恢复、交互历史、Safe Mode 强停与完整 Prompt Injection 防护 |
