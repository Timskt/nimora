...
登录启动属于用户级、默认关闭的系统偏好。macOS 包必须验证签名/公证后的 LaunchAgent 可创建、重登生效、关闭和卸载清理；Windows 安装包必须验证 per-user 登录项、组织策略拒绝和卸载清理。升级不得擅自开启、重置或复制该偏好，应用标识变化必须提供显式迁移与回滚方案。便携、只读或企业受管环境无法修改时，界面显示系统实际状态并给出可恢复错误，不降级为未审计脚本或 Shell 命令。

发布包必须保持稳定 Bundle Identifier/Application User Model Identity，因为单实例通道与它绑定。升级、Stable/Preview 并装、Fast User Switching、远程桌面会话和企业虚拟化环境必须验证实例边界；同一产品通道、同一用户会话只允许一个实例，不同发布通道必须使用不同标识和数据目录。CI 不运行高频多实例 GUI 测试，相关场景在里程碑候选安装包上执行一次，以节约 Actions 时长。
