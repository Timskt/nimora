...
桌面普通 Agent、Auto Mode 单轮恢复和后台 Loop 现可接收同一 `ModelReasoningPolicy`。宿主依据持久 Provider 映射解析一次 `ReasoningMapping`，将其同时绑定到 `ProviderStepInput`、`ProviderRequest` 和 Context Anchor；工具确认续跑与 Auto Loop 后续轮次复用该映射。显式策略用于未声明推理能力的 Provider 时在 Worker、网络和费用前失败关闭。运行页只在 Descriptor 声明能力时显示“自动、节省、极致”和具体固定等级，固定等级禁止静默降级；Browser Preview 不伪造推理能力。

尚未完成 Anthropic/本地 Adapter 参数映射、自动 capability discovery、持久授权与默认策略、CLI 选择器及完整映射审计展示。当前 Adaptive 推荐由宿主使用保守的 `medium` 基线，后续动态推荐必须继续受相同 Provider 能力与缓存身份约束。
