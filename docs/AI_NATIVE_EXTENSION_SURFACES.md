...
AI 可把用户目标转化为可版本化的行为契约、状态机、不变量、示例表、非目标和验收证据计划，并在实现变化时指出需求漂移，而不是让聊天记录充当需求真相。

标准产物：`Intent Contract`、`Acceptance Model`、`Invariant Pack`、`Scope Decision Log`。

强制边界：模型不得自行宣布歧义已解决或验收通过；安全、付费、删除和对外发布条件必须结构化确认；验收 Oracle 必须来自用户决定、公开契约或独立确定性规则；需求变更必须显示语义 Diff 并保留旧版本与迁移影响。

### 2.42 用户数据集、标注与评测资产治理

AI 可帮助用户从合法拥有的数据中建立脱敏样本集、标注规范、困难样本、回归集和个人基准，用于 Prompt、路由、检索、分类器或本地模型的持续评测。

标准产物：`Dataset Manifest`、`Labeling Guide`、`Golden Evaluation Set`、`Data Quality Report`。

强制边界：每条样本记录来源、许可、同意、敏感等级、保留期和删除传播；训练集、验证集和测试集防止污染；原文默认不发送给外部模型；AI 建议的标签必须保留置信度和人工复核状态，模型不得读取隐藏测试答案后参与同一评测。

### 2.43 多 Agent 协商、委托与责任交接

AI 可为多个本地或外部 Agent 生成职责边界、任务委托、报价比较、证据交接、冲突仲裁和失败接管规则，让不同模型与专业 Agent 在同一 Goal 下协作而不共享全部权限。

标准产物：`Delegation Contract`、`Agent Team Topology`、`Handoff Bundle`、`Dispute Resolution Policy`。

强制边界：子 Agent 只获得任务最小切片、预算和租约能力；授权不可沿委托链自动继承；同一高风险动作不得通过多数投票绕过用户；责任、费用、数据去向和最终证据必须回绑原 Goal，循环委托、重复工作和相互等待由宿主检测并终止。

### 2.44 兼容层合成与旧生态迁移

AI 可分析第三方插件、脚本、工作流、Prompt 工程或旧版本项目，生成到 Nimora 当前契约的 Adapter、数据迁移、替代能力映射和行为差异报告，降低用户迁移成本而不把外部运行时直接嵌入 Core。

标准产物：`Compatibility Adapter Project`、`Behavior Diff`、`Migration Fixture Pack`、`Unsupported Surface Report`。

强制边界：外部代码只在隔离分析环境中处理；禁止用兼容层长期绕过当前安全模型；未知行为必须失败关闭并列入差异；Adapter 有明确来源版本、支持窗口、资源预算和退役条件，首个稳定版前不为内部开发快照保留伪兼容包袱。

### 2.45 能源、热量与设备寿命感知编排

AI 可依据电池、温度、GPU/CPU 压力、网络类型、碳强度偏好和用户时段生成端云路由、后台任务、模型量化与渲染降级策略，使长期运行的桌面生命体不过度消耗设备。

标准产物：`Energy Policy`、`Thermal-aware Schedule`、`Compute Placement Plan`、`Degradation Profile`。

强制边界：系统热保护和用户省电设置拥有最高优先级；AI 只能在宿主暴露的聚合指标上决策，不能访问任意硬件接口；延迟任务必须保留截止时间与取消语义；任何云端迁移仍需满足数据出境和费用授权，不能以节能为由静默上传。

### 2.46 来源、版权与生成内容可信链

AI 可为代码、模型、纹理、声音、文档和数据生成来源清单、许可证兼容分析、编辑历史、水印策略和发布披露，帮助个人与创作者安全复用和商业化生成内容。

标准产物：`Provenance Graph`、`License Compatibility Report`、`Attribution Pack`、`Generated Content Disclosure`。

强制边界：来源不明不能被 AI 推断成可商用；许可证结论必须展示依据和不确定项；不得移除第三方署名、水印或权利元数据；发布前由确定性扫描器核对文件摘要、SBOM、引用和许可策略，Secret 与私有来源不得进入公开包。

### 2.47 应急模式与关键任务连续性

AI 可为 Provider 中断、网络封锁、账户失效、扩展撤回、磁盘不足、设备损坏和用户暂时无法操作等情况生成本地降级、替代 Provider、只读恢复、人工接管和数据导出方案。

标准产物：`Continuity Plan`、`Provider Exit Runbook`、`Emergency Offline Pack`、`Recovery Exercise`。

强制边界：应急模式不能扩大权限或关闭审计；关键自动化必须定义最大无人值守时长和安全停止态；恢复演练使用模拟副作用；替代 Provider 不得自动获得原 Provider 的 Secret、缓存或数据授权，任何不可确认副作用保持 `indeterminate` 并等待人工裁决。

### 2.48 个人决策记忆与可撤销偏好学习

AI 可从用户明确批准的选择中提炼可查看的决策规则、例外、置信度和过期条件，用于减少重复询问，同时允许用户知道“为什么这样做”、临时覆盖、局部重置或全部删除。

标准产物：`Decision Preference Pack`、`Exception Ledger`、`Preference Expiry Policy`、`Decision Explanation Template`。

强制边界：沉默、疲劳点击和历史高权限批准不能自动形成永久偏好；安全确认、身份验证和新数据目的不可被学习规则跳过；敏感偏好默认本地保存且不参与跨用户训练；规则冲突时选择更保守策略，并把推断与用户明确陈述分开显示。
