...
当前实现已具备任务状态机、Provider/Tool 单步、参数绑定批准、预算、取消、历史、CLI JSON 契约和受控模块调用基础。`agent-runtime` 已实现独立 Goal/Plan 领域契约：Goal 生命周期与可修订 Plan 分离，Plan 修订保持单调，完成状态要求当前 Plan 每一步均有证据；SQLite 使用 Goal 当前状态表与不可变 Plan 修订表事务持久化，并拒绝陈旧写入和索引元数据/Payload 不一致。CLI 已提供 `goal create|list|show`、`goal plan replace` 与 `goal status set` 的稳定 JSON 接口，可跨进程恢复。Auto Mode 已实现有界 Policy、Goal/Plan/Workspace 精确绑定、逐步准入决策、预算累积、结构化暂停原因、SQLite 乐观并发仓储、重启强制暂停，以及 `goal auto start|status|pause|resume|cancel` 机器可读控制面。宿主无关的 `AutoModeTurnSupervisor` 已接入现有 `AgentCoordinator`：每轮先准入 Provider 步骤，再对整批 Tool 做零副作用预检；只有全部调用均为获准安全只读时，才通过原 Tool Registry 与 Backend 执行并生成严格关联 continuation。版本化 `AutoModeCheckpoint` 与 SQLite CAS 仓储已实现有界 Provider continuation、Task/Goal/Plan/Workspace/Policy 精确绑定、序号单调和索引元数据复验，不保存批准或宿主对象。独立 `agent-auto-host` 已新增恢复应用服务：跨 Goal、Session、Checkpoint 与 Workspace 四类持久状态复验精确绑定，重新执行有界 Workspace 扫描，只释放暂停状态的 Task 和 continuation 数据，不调用 Provider/Tool，也无法恢复旧 Approval。结构化 Context Compactor 已实现 Goal/约束/待办/证据/Workspace/Plan Anchor、可信系统消息强制保留、Tool Call/Result 原子单元、源摘要和预算不足 fail-closed；内容寻址 Cache 已实现 Provider/模型/Plan/Workspace/消息绑定。内存层具备 TTL、LRU 和容量治理；SQLite 持久层进一步具备事务化 TTL 清理、条目/总字节 LRU 淘汰、数据敏感等级命中门禁、索引/Payload 复验和按 Workspace fingerprint 显式失效。不可变 Workspace Snapshot 已实现规范化相对路径、文件 SHA-256、大小/可执行位、父指纹 revision 链和 Added/Modified/Deleted 漂移集。独立 `agent-workspace-host` 已实现 canonical root、禁止跟随 symlink、文件数/单文件/总字节/深度/墙钟边界、Git/通用/Nimora ignore、读取前后 metadata 与文件身份复核；Git Adapter 通过无 Shell 的有界子进程读取 HEAD commit/tree、index tree、分支 ahead/behind 和 staged/unstaged/untracked/conflict 集合。CLI 已提供 `ai workspace inspect` JSON 控制面，只输出相对路径和不可逆 root fingerprint。Workspace Snapshot 现已通过 SQLite 保存单调 revision 链并复验索引/Payload；`goal auto start` 只接受真实 `workspaceRoot`，扫描 revision 1 后绑定指纹，`resume` 重新扫描同一根目录，内容漂移时持久化后继快照并以 `workspace-changed` 保持暂停，不再信任调用方伪造 revision。桌面 Capability Gateway 持久循环、恢复候选到显式 Resume/Provider 下一轮的原子提交、持久 Cache 的 Auto Host Loop 接入与加密密钥服务、Auto Mode 桌面每轮扫描、双仓储原子应用服务、多 Agent 调度、桌面 Goal UI 与完整终端交互仍是明确实施缺口，不得以单轮 Supervisor、普通 Agent Task 或 Automation Run 冒充完成。
...
恢复候选现在必须经过显式 Resume：Session `updated_at_ms` 与 Checkpoint `sequence` 双 CAS 在单一 SQLite Immediate 事务内同时提交 Session 与 Task 的 `running` 状态。任一陈旧或并发写入都会整体回滚；提交阶段不调用 Provider/Tool，也不恢复旧 Approval。每轮执行前必须先创建不可过期重领的 durable Turn Attempt，精确绑定 Session、Checkpoint sequence、Session timestamp 与请求指纹；同一 Session 只能存在一个 Attempt。每轮执行结果把 Continue、Paused 或 Completed 的 Session/Task 生命周期、完整 Provider continuation 与单调 Checkpoint sequence 在同一 SQLite Immediate 事务中双 CAS 提交，并原子消费对应 Attempt；终态 Assistant 文本随 Checkpoint 保存，批准证明与待执行写调用不进入持久 continuation。远程 Provider 或 Tool 已经执行后若提交失败，Attempt 被隔离为 `indeterminate`，Host 返回明确的 `CommitIndeterminate`；崩溃恢复发现 active/indeterminate Attempt 时同样不释放 continuation，必须交由恢复流程或用户检查，禁止自动重放未知结果。后续缺口收窄为该链路与桌面持久监督循环的实际执行接线、人工处置/对账控制面、系统密钥加密与治理 UI。
...
| Auto Mode | 在 Tool、Capability、风险、数据、费用、时间、步骤和并发预算交集内自动推进 | Supervisor 领域、SQLite 会话、CLI 控制面、安全单轮协调、桌面有界后台循环及 Exit/Safe Mode 排空已实现 | 崩溃投影恢复、桌面控制 UI 与真实执行端到端验证 |
...
| 文件追踪与版本 | 文件内容、可执行位、清单 revision 和父指纹形成不可变版本链；路径级漂移可审计 | Snapshot/ChangeSet、SQLite 单调版本链、宿主有界扫描、ignore、TOCTOU 复核、Git Adapter、CLI 检查以及 Auto 启动/恢复漂移阻断已实现 | 桌面 Auto Mode 每轮接入、快照保留/清理、显式 Session 重绑定、内容相同重命名归并、子模块/LFS 策略和桌面治理 UI |
...
首个生产工具目录位于 `crates/agent-tools`，当前公开十四项工具：`asset.catalog.read`、`automation.definition.validate`、`character.state.read`、`pet.action.catalog.read`、`pet.state.read`、`profile.state.read`、`program.catalog.read`、`runtime.health.read`、`pet.animation.play`、`pet.care.perform`、`pet.position.move`、`profile.active.switch`、`character.active.switch` 与 `program.installed.execute`。目录只包含 Tool Descriptor 和固定模块 Adapter，不暴露 `DesktopState`、Repository、Tauri Command、任意命令字符串或文件路径。只读授权使用可扩展能力集合；自动化验证接收有界定义与测试事件，只调用 `AutomationEngine` 的 Dry-run 模式并返回 `planned`、不匹配或校验失败结果，Backend 若收到任何真实 Command 会失败；资源目录只返回已验证资产摘要，角色状态只返回当前 Asset ID、渲染后端、画布/锚点和能力布尔值，主动剔除模型路径与资源 URL；动作目录直接从 Runtime 的 `PetAction` 当前词汇生成，并指明对应写工具与参数，避免 Provider 猜测动作；程序目录只返回完整性复验通过的已安装程序身份、Manifest 声明、预算与精确版本授权摘要，损坏项只计数，不返回源码、安装路径、Worker 路径或宿主句柄；运行健康只返回启动、安全、Outbox 与备份摘要，不包含日志、用户正文、路径或密钥。`pet.care.perform` 只接受 `feed`、`play`、`groom`，以不可逆写入建模并要求 Low 风险确认；执行统一进入 Runtime Host，复用宿主时钟、冷却、原子持久化与 Safe/Recovery 门禁。
...
CLI 已接入 `provider:ollama-loopback`，但只接受经 `nimora.provider-worker-manifest/1` 验证且显式声明 Ollama loopback 与 OpenAI-compatible 能力的通用 Agent Provider Worker。调用方必须同时提供 `--sidecar-root` 与由宿主或发行系统信任的 `--sidecar-manifest-sha256`；Manifest 和 Worker 均拒绝符号链接、路径逃逸、能力缺失或乱序、大小或 SHA-256 不匹配。构建脚本生成的 `.sha256` 只是供 CI、签名或宿主嵌入信任锚使用的发布素材，不能从同一可写目录读取后自行信任，也不等价于发布者数字签名。
...
桌面正常启动现执行持久 Job 投影重建：SQLite Running Session 先转为 `paused/restarted`，任何遗留 Active Attempt 再永久隔离为 `indeterminate`；内存 Supervisor 仅导入终态历史，不占用 Session、不创建 Runner、不调用 Provider/Tool。`auto_mode_job_history` 返回当前进程产生和重启恢复的统一快照；浏览器预览继续稳定拒绝该宿主能力。可恢复集合使用专门查询并带硬上限，超过上限启动失败进入恢复路径，禁止因普通历史过多而静默漏掉旧 Attempt。
...
生产桌面构建会嵌入 `agent-provider-worker.json` 的 SHA-256 信任摘要。启动时宿主仅从受控资源候选目录发现 Worker，并复用 CLI 的 Manifest 路径、能力集合、Manifest 摘要、普通文件、大小和 Worker 摘要校验；全部通过后才把 Worker 声明并由宿主配置启用的 Provider 注册到同一 `ProviderRegistry`。工作台只展示 Registry 中真实可用的 Provider，任务显式携带 Provider ID 与模型名，未知 Provider、空模型和越界模型名在调用前拒绝。
