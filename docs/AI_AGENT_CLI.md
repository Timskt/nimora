...
| Auto Mode | 在 Tool、Capability、风险、数据、费用、时间、步骤和并发预算交集内自动推进 | Supervisor 领域、SQLite 会话、CLI 控制面、安全单轮协调、桌面有界后台循环及 2 秒退出排空已实现 | 崩溃投影恢复、Safe/Recovery 收敛、桌面控制 UI 与真实执行端到端验证 |
