...
| 文件追踪与版本 | 文件内容、可执行位、清单 revision 和父指纹形成不可变版本链；路径级漂移可审计 | Snapshot/ChangeSet、宿主有界扫描、ignore、TOCTOU 复核、Git HEAD/index/worktree Adapter 与 CLI 检查已实现 | Workspace 持久仓储、Auto Mode 每轮接入、内容相同重命名归并、子模块/LFS 策略和桌面治理 UI |
