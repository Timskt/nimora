...
当前自动化证据：领域测试验证首次延迟、确定性 Observe 动作、动作截止、冷却、用户拖拽抢占及安静策略即时抑制；Desktop 纯函数测试验证 Profile 五档频率单调性、0% 禁用、Focus、Presentation 与 Offline 语义，以及负坐标副屏、显示器边缘夹取、完全离屏主屏回退、最大交集屏幕选择、分辨率缩小和无显示器枚举；Runtime/Persistence 测试证明状态与标准事件原子持久化和版本 1 Pet Snapshot 可往返。PET-007 的离线 24 小时、PET-010 多屏/DPI、PET-011 8 小时长稳及 PET-016 至 PET-022 的真实原生窗口观感仍需真机执行，不得仅凭单元测试标记通过。
