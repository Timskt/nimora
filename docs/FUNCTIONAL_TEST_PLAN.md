...
当前自动化证据：领域测试验证首次延迟、确定性 Observe 动作、动作截止、冷却及用户拖拽抢占；Desktop 纯函数测试验证负坐标副屏与显示器边缘夹取；Runtime/Persistence 测试证明状态与标准事件原子持久化和版本 1 Pet Snapshot 可往返。PET-007 的离线 24 小时、PET-010 多屏/DPI、PET-011 8 小时长稳和 PET-016/017 的真实原生窗口观感仍需真机执行，不得仅凭单元测试标记通过。
