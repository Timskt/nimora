...
- **批准不等于资源准入**：安装批准、运行参数批准和执行时资源租约彼此独立；真实执行必须在 Run Journal、活跃运行与 Backend 之前原子获取 SQLite 租约。
- **等待批准不占运行租约**：Medium/High 动作等待用户决定时不消耗并发名额或启动冷却；一次性批准被 claim 后才重新执行准入。
- **预留不等于实际费用**：Agent 子任务按 `maxCostMicrounits` 预留 Automation 当日预算，Completed 仅按 Provider 累计 Usage 结算，二者必须在审计与 UI 中分别展示。
- **未知费用不等于零**：Provider 结果不确定、宿主崩溃或恢复发现未结算预留时进入 `indeterminate`，继续占用完整预留，直到人工或受信对账流程给出可审计结论。
- **重启不能绕过治理**：启动恢复清理失效进程租约，但保留持久冷却；并发、冷却、预算竞争和幂等结算均由 SQLite 事务裁决，不能依赖单进程内存计数。

## AI 辅助创作边界

桌面“扩展”工作区可让外接 AI 生成 User Program、Skill 与 Automation 的结构化草案。入口使用空 Tool allowlist、`AgentAutonomy::Draft`、固定深度和受限预算；模型输出经过严格 JSON、生产契约、独立 Worker 或 DryRun。审查生成草案摘要、当前安装版本、Capability/作用域 Diff 与风险；批准绑定草案和完整审查摘要，安装前重审，基线变化即失败关闭。User Program/Skill 复用生产包安装器；Automation 定义要求三段数字版本并进入 SQLite 原子 Catalog。Automation 首装、升级和回滚均保持停用，普通安装只接受严格递增版本并保存一个上一版本；同版本覆盖或隐式降级失败，降级只能走显式回滚。工作区提供目录、启停和回滚。当前 Diff 覆盖 Capability/命令新增移除、Program/Skill 声明范围和 Automation 触发器、条件、动作参数、补偿与失败策略变化。真实运行历史使用 `(startedAtMs, runId)` 稳定游标翻页；事件会话健康快照只暴露活跃状态与 executed/dropped/failures 饱和计数，不暴露事件正文。Automation 每次真实运行先用生产 Engine 做零副作用判定，再以宿主注册策略计算主动作和补偿动作的有效风险；任一 Medium/High 会把完整参数、事件快照、定义版本和预分配 `runId` 写入五分钟一次性 SQLite 批准计划，且此前不创建 Run Journal、不注册活跃运行、不调用 Backend。批准时原子 claim 并重算风险；安装版本、参数或宿主策略漂移均失败关闭，拒绝、过期和重复处理不能执行，Critical 不进入普通批准。批准后才以同一 `runId` 从头执行；重启保留未过期 pending、将 executing 收敛为 interrupted，绝不自动重放副作用。多事件 Mock、文件/依赖/数据迁移 Diff、持久健康趋势与自动隔离仍需继续闭环。完整范围见 [`AI_EXTENSION_FACTORY.md`](AI_EXTENSION_FACTORY.md)。
