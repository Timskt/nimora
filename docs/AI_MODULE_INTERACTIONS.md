...
AI Creator 使用独立 `desktop:creator-user` requester，只允许登记的本地 Provider，Tool 集合恒为空，autonomy 固定为 `Draft`，调用深度固定为 1。宿主把版本化规则作为 Public/Trusted System 消息，把用户需求作为 Personal/Untrusted User 消息；Provider 输出始终不可信。完成响应必须通过 `nimora.creator-draft/1` 严格解析及生产校验。User Program 与 Skill 的 JavaScript 随后通过各自独立 Worker 的 `Validate/Validated` 协议做 parse-only 检查，不执行顶层代码、不注入 SDK、不要求安装前 Active Lease。检查报告使用 `nimora.creator-draft-check/1` 并逐文件返回稳定状态。显式保存时宿主再次校验请求、完整草案和 Worker 检查，再通过 Tauri-free Writer 写入 Workspace 隐藏草案目录；Writer 不覆盖目标、拒绝符号链接，并以同目录 staging rename 发布。保存仍不进入 Capability Gateway，也不安装或启用；后续必须保持“沙箱、预览、Diff、批准、安装、监控、修复、回滚”的阶段隔离。完整能力工厂见 [`AI_EXTENSION_FACTORY.md`](AI_EXTENSION_FACTORY.md)。
