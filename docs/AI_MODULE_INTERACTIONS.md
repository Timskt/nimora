...
Skill Worker 产生的另一类输出是模块 Command 计划。该计划不能直接调用 Desktop 内部函数，也不能借 Agent 身份绕过命令策略。Manifest 的 `commandAllowlist` 必须逐项使用 `safe.*` 注册命令且要求 `invoke-commands` Capability；Desktop 在执行任何副作用前整批预检 Worker 计划、精确 allowlist 和宿主风险注册。已注册的 Safe/Low 命令直接通过 `ModuleGatewayPolicy` 与共享 Capability Gateway 执行；批次包含 Medium/High 命令时，宿主保存不可变的原始 Command 与 Agent Task 计划，返回逐条参数、风险和五分钟有效期，批准后才整批继续，拒绝、过期或重复批准均不能执行。未知或未声明命令仍在任何副作用前拒绝。当前待批准计划只存在桌面进程内，重启即安全失效；SQLite 持久状态、恢复查询和审计回执仍待实现。反方向的模块 Agent Task 不经过 Tool Registry，而是使用租约式 requester 进入 `module-agent-adapter` 和 `AgentTaskGateway`。因此两条链路共享预算、Trace、取消和审计语义，但保留独立的准入职责，避免形成 AI↔模块递归绕过。
