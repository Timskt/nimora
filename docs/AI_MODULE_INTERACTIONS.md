...
| Automation | Dry-run、版本化 Catalog、启停、回滚、运行 | 自然语言生成规则、升级与解释失败 | Event 不直接成为 Prompt；安装前重审，运行仍经 Gateway | Catalog 已实现；真实事件订阅与运行授权待闭环 |
...
本地用户从 Agent 工作区发起且由宿主确认 `Succeeded` 的 Desktop Task 会进入 Pet 协作成长桥。桥只接受精确 `AgentTaskOrigin::Desktop + desktop:local-user`，以 Task UUID 在 Pet 快照内原子去重；Skill、Automation、用户程序、Creator 和前端 Companion Signal 均不能声明任务完成或领取成长。该桥不读取 Prompt、Response、Token 或文件内容，只持久化有界 UUID 收据，并通过 `pet.relationship.collaborated` 发布成长前后数值。
...
AI Creator 使用独立 `desktop:creator-user` requester、空 Tool 集合、`Draft` autonomy 和深度 1；Provider 输出通过严格草案契约和独立 Worker/DryRun。升级审查从当前安装版本计算 `added/removed/scope-changed`，批准绑定草案与完整审查摘要，安装前重审。User Program/Skill 复用生产包安装器；Automation 使用版本化 SQLite Catalog，事务升级保存上一版并强制停用，目录启停与回滚不能绕过 Safe/Recovery 门禁。Automation 运行仍只经共享 Gateway；Catalog 安装批准不等于未来每次高风险动作授权。后续保持多事件 Mock、文件/依赖/迁移 Diff、真实事件订阅、运行授权、监控和修复阶段隔离。完整能力工厂见 [`AI_EXTENSION_FACTORY.md`](AI_EXTENSION_FACTORY.md)。

### 6.5 Automation 参数绑定运行批准
