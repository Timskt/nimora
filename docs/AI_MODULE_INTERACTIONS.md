...
准入拒绝使用稳定 `snake_case` 原因枚举：`invalid_policy`、`segment_budget`、`byte_budget`、`invalid_source`、`invalid_segment`、`prompt_injection`。共享结果只携带脱敏来源类别、段数和累计字节，不携带来源完整标识或任何正文。Automation 宿主在返回永久 `ActionFailure` 前必须把该结果与 Run、Trace、Automation、Action 和 Command Execution ID 写入安全诊断 Journal；Journal 写入失败时继续拒绝，绝不能因可观测性故障放行 Provider 或 Submitter。
...
`crates/skill-runtime` 已补充 Skill 侧准入前置契约：只有 Manifest 同时声明 Agent Task Contribution 与 `invoke-agent-tasks`、用户对当前精确版本授予完全一致的 Capability 集合、并且 Skill 当前已激活时，Host 才返回可信 `skill:<id>` requester。暂停、崩溃、quarantine 和卸载均撤销 Contribution 快照与 requester。宿主随后必须用该 requester 构造 `ModuleAgentAdapter`；Skill Runtime 本身不依赖 Provider、Task Submitter、Tool Registry 或数据库。Desktop 已实现 Skill 包安装、目录、精确授权、启停、回滚、启动复验恢复和 `execute_skill` IPC：每次执行重新加载完整性复验后的 active 包，以 Host 当前 Activated Manifest 作为精确 Worker 租约，并将 Agent Task 计划用租约式 `skill:<id>` requester 接入共享 Module Adapter、Provider 执行和 Agent History。Skill 执行自身另有 metadata-only History，覆盖等待批准、完成、拒绝和失败状态，并提供分页与隐私删除 IPC；它不复制 Agent History 正文，也不把批准 Journal 当作永久历史。Connector Runtime 尚未实现，不得复制桌面逻辑或直连 Provider。

Skill Worker 产生的另一类输出是模块 Command 计划。该计划不能直接调用 Desktop 内部函数，也不能借 Agent 身份绕过命令策略。Manifest 的 `commandAllowlist` 必须逐项使用 `safe.*` 注册命令且要求 `invoke-commands` Capability；Desktop 在执行任何副作用前整批预检 Worker 计划、精确 allowlist 和宿主风险注册。已注册的 Safe/Low 命令直接通过 `ModuleGatewayPolicy` 与共享 Capability Gateway 执行；批次包含 Medium/High 命令时，宿主将不可变的原始 Command 与 Agent Task 计划写入 SQLite，返回逐条参数、风险和五分钟有效期，批准后才整批继续，拒绝、过期或重复批准均不能执行。Journal 使用 `pending → executing → completed|failed` 的原子 claim，以及单事务 `pending → rejected|expired`；启动恢复保留未过期 pending、将遗留 executing 标记 interrupted，并通过列表 IPC 恢复用户决策界面。未知或未声明命令仍在任何副作用前拒绝。反方向的模块 Agent Task 不经过 Tool Registry，而是使用租约式 requester 进入 `module-agent-adapter` 和 `AgentTaskGateway`。因此两条链路共享预算、Trace、取消和审计语义，但保留独立的准入职责，避免形成 AI↔模块递归绕过。
...
- Skill 执行现在由宿主活跃注册表关联独立 Worker 取消令牌与当前 Provider `CancellationFlag`；用户按 `execution_id` 取消后，Worker、Command 间检查和当前 Agent step 同步停止，历史立即收敛为不可覆盖的 `cancelled`。等待批准的计划仍使用 `reject`，避免混淆“尚未开始”和“正在执行”。
- 获准且 Activated 的 Skill `onEvent:*` 声明由 Desktop 转换为 Runtime Event Bus 的宿主订阅；每个 Skill 使用独立 32 项有界队列和串行调度线程，事件以版本化只读 JSON 输入进入 Worker。Host 重建、停用、升级、回滚、Safe Mode 或线程故障会撤销订阅并取消关联 Worker/Provider；代际 ID 防止旧线程迟到退出误删新会话。
...
- Desktop 对话、Desktop 独立 Tool 准备入口和 CLI 非交互任务均已迁移到 `AgentTaskGateway`；其 Provider 与 Tool 集合从当前生产目录生成，入口不能通过自定义请求扩大策略。Automation、User Program 与 Skill 已有生产入口；Skill 经独立 Worker、可信 Manifest 租约和 Desktop Module Submitter 调用 AI，且取消可传播至 Provider。Connector 尚无生产入口，因此模块反向调用仍未全部贯通。
- Tool Registry 已支持获准 Activated Skill 的动态 Agent Tool Contribution：Manifest 负责命名空间、Schema、Capability 和精确命令映射，Desktop 负责与宿主命令风险交叉复核并在每次 Catalog、Provider Turn 和批准续跑时重建租约快照。Connector Contribution 尚未接入；新增模块不得直接把工具硬编码进 Provider Adapter。
...
4. 为 Connector 实现与 Skill 相同的可信租约、独立 Worker、Context Admission、Desktop Module Submitter 和取消传播入口。
5. 扩展 Automation 保存/启停/运行工具，形成“AI 生成并安全保存规则”闭环。
6. 将后续 Connector Contribution 按 Skill 已实现的动态租约、风险复核和 Gateway 映射模式汇入共享 Registry。
7. 增加 Files、Clipboard、Notification、Calendar、Memory 和 Vision 的窄能力 Adapter。
8. 完成任务持久恢复、递归检测、全链路审计与跨平台故障测试。

## 13. AI Creator 专用链路

AI Creator 使用独立 `desktop:creator-user` requester，只允许登记的本地 Provider，Tool 集合恒为空，autonomy 固定为 `Draft`，调用深度固定为 1。宿主把版本化规则作为 Public/Trusted System 消息，把用户需求作为 Personal/Untrusted User 消息；Provider 输出始终不可信。完成响应必须通过 `nimora.creator-draft/1` 严格解析及生产校验。User Program 与 Skill 的 JavaScript 先通过各自独立 Worker 的 `Validate/Validated` 协议做 parse-only 检查，不执行顶层代码；随后进入无真实副作用行为沙箱。User Program 使用 `Sandbox/Sandboxed` 且只获得只读输入，Skill 只记录 Command/Agent Task 计划，Automation 使用拒绝真实命令的 DryRun Backend。检查报告使用 `nimora.creator-draft-check/1` 并逐文件返回稳定状态；升级审查从完整性复验后的当前安装 Manifest 计算 Capability `added/removed` 与声明作用域 `scope-changed`。批准绑定草案 SHA-256 和完整审查报告 SHA-256，安装前重新审查；当前安装版本或 Diff 漂移会原子消费失配凭证并要求重新批准。显式保存时宿主再次校验请求、完整草案、语法与行为检查，再通过 Tauri-free Writer 写入 Workspace 隐藏草案目录；Writer 不覆盖目标、拒绝符号链接，并以同目录 staging rename 发布。User Program/Skill 安装复用生产原子安装器且保持未授权、未启用；后续仍保持“多事件 Mock、文件/依赖/迁移 Diff、预览、授权启用、监控、修复、回滚”的阶段隔离。完整能力工厂见 [`AI_EXTENSION_FACTORY.md`](AI_EXTENSION_FACTORY.md)。
