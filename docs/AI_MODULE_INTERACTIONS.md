...
Skill Worker 产生的另一类输出是模块 Command 计划。该计划不能直接调用 Desktop 内部函数，也不能借 Agent 身份绕过命令策略；宿主必须先把 Contribution 解析到共享 Command Registry，再经过参数 Schema、风险上调、用户批准、Capability Gateway 和审计后执行。当前 Desktop 在该 Registry 接线完成前对任何非空 Skill Command 计划整体 fail-closed，不执行部分 Agent 结果或任何命令副作用。反方向的模块 Agent Task 不经过 Tool Registry，而是使用租约式 requester 进入 `module-agent-adapter` 和 `AgentTaskGateway`。因此两条链路共享预算、Trace、取消和审计语义，但保留独立的准入职责，避免形成 AI↔模块递归绕过。
