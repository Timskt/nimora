...
当前 `crates/skill-runtime` 已实现首个宿主无关生命周期核心：`nimora.skill/1` Manifest 严格校验、命名空间 Contribution、精确版本与精确 Capability 授权、激活快照、暂停撤销、显式崩溃恢复、五分钟三次崩溃 quarantine、用户显式解除隔离和非活跃卸载。该核心不执行第三方代码；包事务安装、文件完整性与签名、独立 Skill Worker/Supervisor、IPC、资源配额和桌面管理 UI 仍必须在其外层实现，不能用进程内调用替代隔离 Host。
