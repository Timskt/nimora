...
`crates/skill-package` 与 Desktop 已实现原子安装、宿主生成完整性锁、完整库存与 SHA-256 复验、旧 active 备份和回滚，以及安装目录、持久授权状态和 Runtime Host 的桌面管理 IPC。安装、升级和回滚均不自动授权；授权精确绑定 `skill_id + version + capabilities`，启用与授权分离。正常启动重新复验包与持久状态后重建 Host，损坏、丢失或状态不匹配的 Skill 不恢复；Recovery Mode 使用隔离目录、内存状态和空 Host。尚未实现跨文件系统与 SQLite 的崩溃一致性 Journal、发布者签名、操作系统级 CPU/内存沙箱、Desktop Worker 执行入口、Command/Agent Task Submitter 接线和管理 UI。
