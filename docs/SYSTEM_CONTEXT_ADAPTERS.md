...
> 状态：领域策略、Desktop Coordinator、用户覆盖与 macOS 全屏 Adapter 已实现；其他原生 Adapter 尚未实现
...
- 全屏：使用 macOS Accessibility `AXFullScreen` 查询前台窗口，只读取布尔属性，不读取标题、描述或内容；子进程硬超时后强制回收。Accessibility 权限缺失时健康状态显示“不可用/降级”，信号自然过期并回到 Profile，而不是假装检测成功。
...
当前 Desktop Coordinator 已把活动 Profile、用户覆盖、Safe Mode 和情境决策统一合并到可逆窗口事务；控制中心提供“自动避让 / 始终显示 / 始终隐藏”，托盘恢复也不能绕过屏幕共享隐私。macOS 全屏 Adapter 已接线；屏幕共享、免打扰、游戏及 Windows Adapter 尚未实现，因此不得宣称完整跨平台自动感知完成。
