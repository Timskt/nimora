...
- 每个里程碑产生可安装、可演示、可重置的开发成果；首个稳定版发布后，发布候选才必须验证版本回滚。
- 先冻结契约和测试夹具，再实现多个适配器。
- 平台相关功能必须在 Windows 和 macOS 分别验收。
- 安全、诊断和卸载不是发布后的补充工作；数据迁移只在存在真实已发布版本后设计。
...
| 层级 | 覆盖内容 |
|---|---|
| Unit | FSM、策略、合并、原子回滚、过滤、风险计算 |
| Schema | 当前契约正反例、未知版本拒绝、严格字段校验 |
...
- PR 合并标题必须符合 Conventional Commits。首个稳定版前的破坏性变化直接更新唯一当前契约、测试夹具和文档，不保留兼容层；稳定版发布后的破坏性变化才必须附迁移、兼容和回滚说明。

- 公开 Schema 100% 具有正例和反例测试。
- Core 新代码单元覆盖率不低于 80%，安全策略分支 100%。
- 不存在 Critical/High 依赖漏洞或未处理的高危安全发现。
- 空闲性能、启动时间、包体和 soak test 达到产品 NFR。
- 所有新增公共接口有唯一当前版本、文档和示例；首个稳定版前直接修改该契约，不建立弃用层。
...
- GLB Renderer 发布门禁必须覆盖受控资源协议、真实 WebGL 加载、自动 framing、减少动画、GPU context 丢失、连续切换与完整资源释放；元数据验证、签名认证、进程隔离和 OS/GPU 沙箱必须分别报告，不得互相替代。
- Desktop 生产构建必须生成 Vite Manifest 并执行 Bundle Budget：主入口原始体积不超过 350,000 bytes，GLTF/Three.js 只能作为直接动态入口且不超过 650,000 bytes；边界消失、回归为同步依赖或超预算都必须使本地构建和 CI 失败。
