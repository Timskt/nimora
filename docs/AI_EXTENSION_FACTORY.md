...
当前已实现 AI 生成 User Program、Skill、Automation 的严格结构化草案，使用专用 Creator Agent、空 Tool allowlist、生产契约复验和 Workspace 原子保存。User Program 与 Skill 草案的 JavaScript 已具备独立 Worker parse-only 检查，保存 IPC 会重新检查。

尚未完成沙箱行为测试、权限与行为 Diff、风险批准、安装启用、Connector/Agent/UI/资产生成、自动修复、运行监控和回滚闭环。它们属于完整产品范围，必须按本规范逐项实现，不能用“未来版本”作为删除理由。
