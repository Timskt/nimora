...
当前已实现 AI 生成 User Program、Skill、Automation 的严格结构化草案，使用专用 Creator Agent、空 Tool allowlist、生产契约复验和 Workspace 原子保存。User Program 与 Skill JavaScript 已具备独立 Worker parse-only 与无真实副作用行为沙箱，Automation 使用生产 DryRun。审查返回摘要绑定的 Capability Diff 与最高风险；五分钟一次性批准可被保存或安装原子消费。User Program 与 Skill 复用生产包安装器；Automation 使用版本化 SQLite Catalog。三类安装均强制未授权、未启用并保留上一版本，Automation 已有目录启停与回滚 UI。

Creator 审查会复验当前已安装版本，并比较 Capability/命令新增与移除，以及 Program、Skill 和 Automation 行为作用域变化；批准同时绑定草案与完整审查摘要，安装基线漂移会失败关闭。Automation Catalog 已实现原子安装、同版本拒绝、升级保留上一版、强制停用、显式启停和双向回滚。尚未完成多事件 Mock、文件/依赖/数据迁移 Diff、Automation 运行期授权与真实事件订阅、删除/历史 UI、Connector/Agent/UI/资产生成、自动修复和运行监控闭环。
