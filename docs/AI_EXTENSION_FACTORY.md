...
当前已实现 AI 生成 User Program、Skill、Automation 的严格结构化草案，使用专用 Creator Agent、空 Tool allowlist、生产契约复验和 Workspace 原子保存。User Program 与 Skill JavaScript 已具备独立 Worker parse-only 与无真实副作用行为沙箱，Automation 使用生产 DryRun。审查返回摘要绑定的 Capability Diff 与最高风险；五分钟一次性批准可被保存或安装原子消费。User Program 与 Skill 可由宿主生成受控临时包并复用生产完整性锁、原子安装和上一版本保留，安装后保持未授权、未启用。

Creator 审查现在会复验当前已安装 User Program/Skill，并比较 Capability 新增、移除，以及事件、命令、Agent Tool Contribution 和运行预算的作用域变化；批准同时绑定草案摘要和完整审查摘要，安装基线漂移会失败关闭；升级安装会撤销旧授权状态。尚未完成多事件 Mock、文件/依赖/数据迁移 Diff、Automation Catalog、Creator 授权启用、Connector/Agent/UI/资产生成、自动修复、运行监控和回滚 UI 闭环。它们属于完整产品范围，必须按本规范逐项实现，不能用“未来版本”作为删除理由。
