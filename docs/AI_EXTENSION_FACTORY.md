...
尚未完成多事件 Mock、已安装版本的 removed/scope-changed/依赖/数据迁移 Diff、Automation Catalog、Creator 授权启用、Connector/Agent/UI/资产生成、自动修复、运行监控和回滚 UI 闭环。它们属于完整产品范围，必须按本规范逐项实现，不能用“未来版本”作为删除理由。
