...
Creator 审查会复验当前已安装版本，并比较 Capability/命令新增与移除，以及 Program、Skill 和 Automation 行为作用域变化；批准同时绑定草案与完整审查摘要，安装基线漂移会失败关闭。Automation Catalog 已实现原子安装、同版本拒绝、升级保留上一版、强制停用、显式启停和双向回滚；已启用项具备真实事件订阅、稳定游标历史、终态删除和活跃会话 executed/dropped/failures 健康快照。尚未完成多事件 Mock、文件/依赖/数据迁移 Diff、Automation Medium/High 参数级运行期授权、Connector/Agent/UI/资产生成、自动修复、持久监控和自动隔离闭环。
