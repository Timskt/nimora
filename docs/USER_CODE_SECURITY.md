...
- 订阅最多 32 个；单次结构化计划最多 32 个总操作（命令与本地存储操作合计）。
...
一次性计划支持 `storage: [{ type: "read"|"write"|"delete", key, value? }]`。每个程序拥有独立本地命名空间，默认总配额 8 MiB、单值上限 256 KiB；键不允许路径分隔符，写入采用临时文件同步后原子替换，并拒绝符号链接。升级和回滚保留程序数据，卸载删除数据必须经过后续显式产品策略与用户确认。Worker 只能提交结构化操作，不能获得目录、文件句柄或自行拼接路径。
