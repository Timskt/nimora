...
关闭控制中心仅隐藏管理界面，明确退出必须经过托盘“退出 Nimora”或操作系统退出协议。托盘退出先尽力同步保存桌宠最终原生位置，再立即向自主循环与 Pet Window Recovery 发布不可逆 shutdown intent，最后请求 Tauri 退出；即使位置保存失败也要记录托盘失败并继续退出。这样窗口销毁事件不能在退出竞态中启动恢复线程或重建桌宠。全局 `ExitRequested` 再幂等执行同一 shutdown intent，并对 Auto Mode 与 Automation Event Session 做有界静默；退出协议不得等待 AI、网络或用户确认。
