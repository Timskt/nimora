use nimora_agent_provider_worker::{
    ProviderSidecarManifest, SidecarVerificationError, verify_provider_sidecar,
};
...
fn write_fixture(root: &std::path::Path) -> (String, PathBuf) {
    ...
    let manifest = ProviderSidecarManifest {
        spec: "nimora.provider-sidecar/1".to_owned(),
        provider_id: "provider:ollama-loopback".to_owned(),
        protocol_version: 1,
    ...
    fs::write(root.join("ollama-provider.json"), &manifest_bytes).expect("write manifest");
...
fn resolves_only_when_trusted_manifest_and_executable_match() {
    ...
    let verified = verify_provider_sidecar(&root, "ollama-provider.json", &manifest_digest)
...
fn rejects_manifest_and_executable_tampering_independently() {
    ...
    assert_eq!(
        verify_provider_sidecar(&root, "ollama-provider.json", &manifest_digest),
        Err(SidecarVerificationError::ExecutableDigestMismatch)
    );

    let (manifest_digest, _) = write_fixture(&root);
    fs::write(root.join("ollama-provider.json"), b"{}").expect("tamper manifest");
    assert_eq!(
        verify_provider_sidecar(&root, "ollama-provider.json", &manifest_digest),
...
fn rejects_manifest_path_traversal_before_filesystem_access() {
    ...
    assert_eq!(
        verify_provider_sidecar(&root, "../provider.json", &"0".repeat(64)),
...
fn rejects_symbolic_link_executables() {
    ...
    fs::write(root.join("ollama-provider.json"), &manifest_bytes).expect("write manifest");
    assert_eq!(
        verify_provider_sidecar(&root, "ollama-provider.json", &digest(&manifest_bytes)),
