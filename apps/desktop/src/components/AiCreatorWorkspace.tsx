...
export function AiCreatorWorkspace({ disabled }: { disabled: boolean }) {
  ...
  return <section className="ai-creator-workspace">
    ...
    <div className="ai-creator-layout"><form className="ai-creator-form" onSubmit={(event) => { event.preventDefault(); void generate(); }}>
      ...
      {gap && result ? <CapabilityGapPreview disabled={busy || Boolean(saveNotice)} gap={gap} onSave={() => void saveGap()} result={result} saveNotice={saveNotice} /> : null}
...
export function CapabilityGapPreview({ disabled, gap, onSave, result, saveNotice }: { disabled: boolean; gap: NonNullable<CreatorDraftResult["capabilityGap"]>; onSave: () => void; result: CreatorDraftResult; saveNotice: string | null }) {
  const plan = result.compositionPlan;
  return <div className="ai-capability-gap"><div className="ai-draft-heading"><div><small>CAPABILITY GAP · NON-EXECUTABLE</small><h3>{gap.title}</h3></div><i>{gap.platformProposalRequired ? "需要平台提案" : "可用现有替代"}</i></div><p>{gap.summary}</p><div className="ai-gap-verification"><strong>宿主目录已核验</strong><span>精确能力 ID 在当前生产 Catalog Snapshot 中不存在</span><code title={result.catalogDigest}>{result.catalogDigest.slice(0, 22)}…</code><span>语义组合图已绑定，但尚未验证自然语言目标映射</span><code title={result.compositionGraphDigest}>{result.compositionGraphDigest.slice(0, 22)}…</code></div><div className="ai-gap-outcome"><span>用户目标</span><strong>{gap.requestedOutcome}</strong></div><h4>缺失能力</h4>{gap.missingCapabilities.map((item) => <article key={item.capability}><code>{item.capability}</code><span>{item.reason}</span><ul>{item.requiredOperations.map((operation) => <li key={operation}>{operation}</li>)}</ul></article>)}{plan ? <small className="ai-gap-proof">已确定性核验 {plan.missingCapabilities.length} 个缺失 ID；未证明自然语言目标不存在其他组合路径。</small> : null}<h4>最低成本替代方案</h4>{gap.closestAlternatives.length ? gap.closestAlternatives.map((alternative) => <article className="ai-gap-alternative" key={`${alternative.kind}:${alternative.title}`}><strong>{alternative.title}</strong><code>{alternative.kind}</code><span>{alternative.tradeoff}</span></article>) : <p className="ai-empty">当前没有不扩大权限的可靠替代方案</p>}<p className="ai-gap-boundary">此报告不含可执行代码、未知命令或运行授权，不能进入审查、批准和安装流水线；只能作为 Creator Project 事实导出。</p><div className="ai-gap-footer"><small>{result.usage.inputTokens + result.usage.outputTokens} tokens · {result.finishReason}</small><button disabled={disabled} onClick={onSave} type="button">{saveNotice ? "报告已保存" : "保存缺口报告"}</button></div>{saveNotice ? <p className="ai-save-notice">{saveNotice}</p> : null}</div>;
}
