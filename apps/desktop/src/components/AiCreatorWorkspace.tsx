...
const artifactKinds: Array<{ kind: CreatorArtifactKind; title: string; detail: string }> = [
  { kind: "user-program", title: "用户程序", detail: "经 Gateway 调用 safe.pet.directive 与只读面，编排宠物与模块" },
  ...
  { kind: "profile", title: "场景 Profile", detail: "生成工作、专注、创作或离线行为策略" },
];
...
/**
 * Pure derivation of why the "generate draft" action is blocked.
 * Returns a short Chinese reason, or null when generation is allowed.
 * Keeps the disabled button honest instead of silently inert.
 */
export function creatorGenerateBlockReason(state: CreatorGenerateState): string | null {
  if (state.disabled) return "安全或恢复模式下禁止生成草案";
  if (!state.providerReady) return "所选 Provider 尚未就绪，请检查连接或稍候重试";
  if (!state.hasRequirement) return "请先描述目标与验收标准";
  if (!state.hasModel) return "请填写要使用的模型名称";
  if (state.networkProvider && !state.networkAllowed) return "该 Provider 需联网，请先勾选允许本次请求联网";
  if (state.busy) return "正在生成并验证，请稍候";
  return null;
}
...
export function AiCreatorWorkspace({ disabled }: { disabled: boolean }) {
  ...
  async function installDraft() {
    ...
    try {
      ...
      setInstallNotice(kind === "profile" ? `${receipt.artifactId} 已原子创建；当前未切换` : `${receipt.artifactId} ${receipt.version} 已原子安装${receipt.replacedPrevious ? "并保留上一版本" : ""}；${kind === "theme" ? "当前未激活" : "当前未授权、未启用"}`);
      setApproval(null);
  ...
  const generateBlockReason = creatorGenerateBlockReason({
    busy,
    disabled,
    hasRequirement: Boolean(requirement.trim()),
    hasModel: Boolean(model.trim()),
    providerReady: status?.state === "ready",
    networkProvider: activeProvider?.locality === "network",
  ...
  return <section className="ai-creator-workspace">
    ...
    <div className="ai-creator-layout"><form className="ai-creator-form" onSubmit={(event) => { event.preventDefault(); void generate(); }}>
      ...
      {kind === "user-program" ? <UserCodeCapabilityMatrix compact /> : null}
      ...
      <div className="ai-provider-row"><label><span>Provider</span><select value={providerId} onChange={(event) => { setProviderId(event.target.value); setModel(""); setAllowNetwork(false); resetReview(); }}>{catalog?.providers.map((provider) => <option key={provider.id} value={provider.id}>{provider.displayName}</option>)}</select></label><label><span>模型</span><input list="creator-models" value={model} onChange={(event) => { setModel(event.target.value); resetReview(); }} /><datalist id="creator-models">{status?.models.map((item) => <option key={item.name} value={item.name} />)}</datalist></label></div>
      ...
      <button className="primary-button" disabled={Boolean(generateBlockReason)} title={generateBlockReason ?? undefined} type="submit">{busy ? "正在生成并验证…" : "生成安全草案"}</button>
      {generateBlockReason && !error ? <p className="ai-creator-hint" role="status">{generateBlockReason}</p> : null}{error ? <p className="ai-creator-error">{error}</p> : null}
    ...
    <SkillExecutionHistoryPanel disabled={disabled} />
...
export function ProfileDraftPreview({ profile }: { profile: Extract<NonNullable<CreatorDraftResult["draft"]>["artifact"], { kind: "profile" }>["profile"] }) {
  const entries = [
  ...
  return <section className="ai-profile-preview">
    ...
    <div>{entries.map(([label, value]) => <span data-enabled={value === true} key={label}><b>{label}</b>{value == null ? "继承默认" : value ? "开启" : "关闭"}</span>)}</div>
    <footer><span>主动互动频率</span><meter max="100" min="0" value={profile.policy.proactiveFrequency ?? 25} /><b>{profile.policy.proactiveFrequency == null ? "继承默认" : `${profile.policy.proactiveFrequency}%`}</b></footer>
    <p>创建后不会自动切换；请在 Profile 控制中心确认实际窗口与声音效果。</p>
  </section>;
}
