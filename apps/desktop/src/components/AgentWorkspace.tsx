...
import {
  ...
  type AutoModeAttemptResolutionDecision,
  type AwaySummary,
  type ConcreteReasoningEffort,
  type DesktopAutoModeAttemptDetail,
  type DesktopAutoModeControlCenter,
  type LocalAgentResult,
  type ModelReasoningPolicy,
  type ResumeAutoModeTurnRequest,
  type DesktopAutoModeTurnResult,
...
/**
 * Build the resume-auto-mode-turn request that advances exactly one Checkpoint
 * (single-step) for a paused session, without spawning a batch runner. Reuses
 * the same grant/workspace preconditions as the batch resume path.
 */
export function buildStepTurnRequest(options: {
  entry: { session: { id: string }; grant?: { status?: string; workspaceRoot?: string | null } | null };
  offline: boolean;
  reasoningPolicy?: ModelReasoningPolicy | null;
  ...
  const request: ResumeAutoModeTurnRequest = {
    sessionId: options.entry.session.id,
    workspaceRoot,
    offline: options.offline,
  };
...
/**
 * Turn a single-step turn result into a Chinese owner-facing notice: reports
 * the new checkpoint, whether the session paused/completed, and cache reuse.
 */
export function describeAutoModeTurnResult(result: DesktopAutoModeTurnResult): string {
  const cache = result.cacheHit ? " · 命中上下文缓存" : "";
  if (result.status === "completed") return `单步推进完成，会话已收敛到 Checkpoint #${result.checkpointSequence}${cache}`;
  if (result.status === "paused") {
    const reason = result.pauseReason ? `（${pauseReasonLabel(result.pauseReason)}）` : "";
    return `已推进到 Checkpoint #${result.checkpointSequence}，会话再次安全暂停${reason}${cache}`;
  }
  return `已推进到 Checkpoint #${result.checkpointSequence}，会话继续运行${cache}`;
}
...
/** Attempt-resolution decision token → owner-facing Chinese (reconcile dialog + history). */
export function resolutionDecisionLabel(decision: AutoModeAttemptResolutionDecision | string): string {
  return ({
    confirmed_not_executed: "确认未执行",
    accept_external_effect_and_cancel: "接受副作用并取消",
  } as Record<string, string>)[decision] ?? decision;
}

/**
 * One-line reconcile context shown before an irreversible resolution. Surfaces whether the
 * session already has authoritative resolution records so the owner never re-decides blind.
 */
export function attemptResolutionSummary(detail: DesktopAutoModeAttemptDetail | null): string {
  if (!detail) return "对账历史加载中…";
  const count = detail.resolutions.length;
  if (count === 0) return "该会话尚无对账记录，这将是首次不可逆决议。";
  const latest = detail.resolutions.reduce((acc, item) => (item.resolvedAtMs > acc.resolvedAtMs ? item : acc));
  return `已有 ${count} 条对账记录，最近一次：${resolutionDecisionLabel(latest.decision)}（${latest.actor}）。`;
}
...
export function AgentWorkspace({ safeMode, recoveryMode, initialView = "run", onNotice }: AgentWorkspaceProps) {
  ...
  const [attemptDetail, setAttemptDetail] = useState<DesktopAutoModeAttemptDetail | null>(null);
  const [attemptDetailBusy, setAttemptDetailBusy] = useState(false);
  ...
  useEffect(() => {
    if (!pendingControl || pendingControl.kind !== "resolve" || !desktopApi.native) {
      setAttemptDetail(null);
      return;
    }
    let cancelled = false;
    const sessionId = pendingControl.entry.session.id;
    setAttemptDetail(null);
    setAttemptDetailBusy(true);
    void desktopApi
      .autoModeAttemptDetail(sessionId)
      .then((detail) => {
        if (!cancelled) setAttemptDetail(detail);
      })
      .catch(() => {
        if (!cancelled) setAttemptDetail(null);
      })
      .finally(() => {
        if (!cancelled) setAttemptDetailBusy(false);
      });
    return () => {
      cancelled = true;
    };
  ...
  async function executeControl() {
    ...
    try {
      ...
      } else if (pendingControl.kind === "step") {
        const request = buildStepTurnRequest({
          entry: pendingControl.entry,
          offline: offlineMode,
          reasoningPolicy: reasoningCapabilities ? reasoningPolicyForChoice(reasoningChoice) : null,
        });
        const result = await desktopApi.resumeAutoModeTurn(request);
        await publishCompanionStatus(result.status === "completed" ? "completed" : result.status === "paused" ? "waiting_for_confirmation" : "running", pendingControl.entry.job.jobId).catch(() => undefined);
        onNotice(describeAutoModeTurnResult(result));
      } else {
