...
export function CapabilityProposalGovernance({ disabled }: { disabled: boolean }) {
  ...
  const [records, setRecords] = useState<CapabilityProposalRecord[]>([]);
  ...
  async function openWorkspace() {
    ...
    try {
      ...
      setRecords(await desktopApi.capabilityProposalQueue(selected));
    } catch (reason) {
  ...
  async function refresh() {
    ...
    setError(null);
    try {
      setRecords(await desktopApi.capabilityProposalQueue(workspaceRoot));
    } catch (reason) {
  ...
  async function review(
    ...
    try {
      ...
      setRecords((current) => current.map((item) => item.proposalId === reviewed.proposalId ? reviewed : item));
    } catch (reasonValue) {
  ...
  return <section className="capability-proposal-governance">
    ...
    {workspaceRoot && records.length === 0 ? <div className="capability-proposal-empty"><span>✓</span><strong>当前没有能力提案</strong><small>队列只读取经过宿主双重核验和内容摘要校验的记录。</small></div> : null}
