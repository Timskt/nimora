...
fn start_auto_mode_job(
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || run_auto_mode_job(&app, job_id, &request, &control))
        .map_err(|error| {
...
fn run_auto_mode_job(
    ...
    let result = run_auto_mode_job_inner(&state, job_id, request, control);
...
#[expect(
    ...
    reason = "temporary desktop application service orchestration pending module extraction"
)]
fn run_auto_mode_job_inner(
    state: &DesktopState,
    job_id: Uuid,
    request: &StartAutoModeJobRequest,
    control: &auto_mode_jobs::AutoModeJobControlHandle,
) -> Result<(), (AutoModeJobStatus, String)> {
    let database_path = state.database_path.as_ref().ok_or((
        AutoModeJobStatus::Failed,
    ...
    loop {
        match control.requested() {
            AutoModeJobControl::Pause | AutoModeJobControl::Cancel => {
        ...
        let checkpoint_sequence = match &result.stop {
        ...
        match result.stop {
            ...
            AutoModeLoopStop::Paused(paused) => {
                return finish_auto_mode_job(
                    state,
                    job_id,
                    AutoModeJobStatus::Paused,
                    paused.session.pause_reason.map(auto_mode_pause_reason_code),
                );
            }
            AutoModeLoopStop::Completed(_) => {
                return finish_auto_mode_job(state, job_id, AutoModeJobStatus::Completed, None);
            }
            AutoModeLoopStop::WorkspaceDrift { .. } => {
                return finish_auto_mode_job(
                    state,
...
fn auto_mode_pause_reason_code(reason: AutoModePauseReason) -> String {
    match reason {
